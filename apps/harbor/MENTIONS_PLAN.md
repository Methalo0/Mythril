...
>
> - **`@here` and `@mythril` are tokens too** (`[@here](#here)`,
>   `[@mythril](#mythril)`), not plain text. Slack, Zulip, and Teams tokenize
...
>   `@mythril` into tokens through the ordinary edit path, attributed to the
...
only works with unique, stable handles, which Mythril member ids are not.
...
- **The name stays in the label** (Zulip's choice, not Slack's) because
  Mythril's raw markdown has first-class readers: the user's agent catches up
  ...
  ULID everywhere.
- **`@mythril` and `@here` stay plain text.** They are fixed addresses, not
...
- **Autocomplete inserts nodes** (`mention-autocomplete.tsx`) for members;
  `@mythril` / `@here` stay text. The profile popover's "Mention" seed
