...
import { memberUrl, messageUrl, parseOrgUrl, spaceUrl, assetUrl, type ProposeChangeResult } from '@mythril/spaces-protocol';
...
describe('spaces, invites, membership', () => {
  ...
  it('the /join/<token> page names the space pre-auth and hands the invite to the app', async () => {
    ...
    expect(html).toContain(
      `mythril://open?type=spaces&org=${encodeURIComponent(harbor.service.org.address)}&invite=${encodeURIComponent(inv.body.token)}`,
    );
    ...
    expect(deadHtml).not.toContain('mythril://');
  });
...
describe('assets and the change-set log', () => {
  ...
  it('stale non-overlapping proposals merge; the proposer must adopt mergedContent', async () => {
    ...
    expect(stale.body.changeSet.attribution).toEqual({ memberId: 'gagan', actingMode: 'agent', agentName: 'Mythril' });
  });
...
describe('feed: the stream, threads, and topic annotations', () => {
  ...
  it('every org link opened in a browser lands on a hand-off page that deep-links into the app, without looking anything up', async () => {
    ...
    for (const [link, expected] of cases) {
      ...
      expect(res.headers.get('content-type')).toContain('text/html');
      const html = await res.text();
      expect(html).toContain('mythril://open?type=spaces&');
