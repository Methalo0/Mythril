...
import { mcpTools, type Member, type Message, type Space } from '@mythril/spaces-protocol';
...
async function startForStore(kind: 'memory' | 'postgres'): Promise<void> {
  ...
  ramAgent = await agentClient(harbor, 'dev-ramnique', { agentName: 'Mythril' });
...
describe.each([['memory'], ['postgres']] as const)('agent face parity (%s store)', (storeKind) => {
  ...
  it("edit_message and delete_message are author-only: your person's messages, nothing else", async () => {
    ...
    expect((edit.event as any).edit.by).toEqual({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
  ...
  it('react adds and removes an emoji as your person, folded on the returned message', async () => {
    ...
    expect((reaction.event as any).reaction.by).toEqual({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
  ...
  it("post_message with a poll; vote_poll as an agent is the member's vote; end_poll by the author's agent", async () => {
    ...
    expect(thread.root.author).toEqual({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
    ...
    expect((end.event as any).end.by).toEqual({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
