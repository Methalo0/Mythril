...
import type { ChangeSet, CreateAssetResult, ProposeChangeResult, ReadAssetResult } from '@mythril/spaces-protocol';
...
// ...and as agents on the MCP face (each member's own Mythril).
...
async function startForStore(kind: 'memory' | 'postgres'): Promise<void> {
  ...
  ramniqueAgent = await agentClient(harbor, 'dev-ramnique', { agentName: 'Mythril' });
  gaganAgent = await agentClient(harbor, 'dev-gagan', { agentName: 'Mythril' });
  prakharAgent = await agentClient(harbor, 'dev-prakhar', { agentName: 'Mythril' });
  ramniqueCron = await agentClient(harbor, 'dev-ramnique', { agentName: 'Mythril', scheduled: true });
}
...
describe.each([['memory'], ['postgres']] as const)('§11 — a day in the life of Roadboard (%s store)', (storeKind) => {
  ...
  it('beat 1 — creation: Ramnique makes the space, the agent seeds the roadmap, four invites land', async () => {
    ...
    expect(seeded.changeSet.attribution).toEqual({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
  ...
  it('beat 7 — chat grammar: a thread starts flat in the stream, agents stay silent, @mythril runs only for its own person', async () => {
    ...
    const replied = await ramnique.post(`/v1/spaces/${spaceId}/messages`, {
      threadRoot: rootId,
      body: 'Yes — Customer X moved it for me. @mythril move SSO to P1.',
      actingMode: 'direct',
    });
    ...
    // @mythril resolves to RAMNIQUE's own agent, runs on their machine, lands attributed.
    ...
    expect(thread.body.messages.map((m: any) => m.author)).toEqual([
      ...
      { memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' },
    ]);
  });

  it('beat 8 — housekeeping: the DRI\'s local cron tidies, attributed "(via Mythril, scheduled)"', async () => {
    ...
    expect(tidy.changeSet.attribution).toEqual({ memberId: 'ramnique', actingMode: 'scheduled', agentName: 'Mythril' });
  });
