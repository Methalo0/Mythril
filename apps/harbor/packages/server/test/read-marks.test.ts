...
import type { Message, ServerFrame } from '@mythril/spaces-protocol';
...
describe.each([['memory'], ['postgres']] as const)('read marks (%s store)', (storeKind) => {
  ...
  it("an agent's post does not read the stream for its member", async () => {
    ...
    const r = await harsh.post(`/v1/spaces/${main}/messages`, { body: 'digest', actingMode: 'agent', agentName: 'Mythril' });
    expect(r.status).toBe(200);
