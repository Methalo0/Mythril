...
import { parseMentions, type Message, type ServerFrame, type Space } from '@mythril/spaces-protocol';
...
const msg = (body: string, author = 'harsh', opts: { agent?: boolean; threadRoot?: string } = {}): Message =>
  ({
    ...
    author: { memberId: author, actingMode: opts.agent ? 'agent' : 'direct', ...(opts.agent ? { agentName: 'Mythril' } : {}) },
    body,
    postedAt: new Date().toISOString(),
    offset: 2,
    replyCount: 0,
    reactions: [],
    ...(({ members, here, mythril }) => ({ mentions: members, mentionsHere: here, mentionsMythril: mythril }))(parseMentions(body)),
  }) as Message;
...
describe('classification', () => {
  ...
  it('mention > here > dm > reply > message; only tokens address, tokens in code are cites', () => {
    ...
    expect(classifyFor('gagan', space('shared'), msg('[@mythril](#mythril) summarize'), none)).toBe('message');
  });
