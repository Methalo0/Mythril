...
describe('the grammar', () => {
  it('parses tokens, dedupes ids, and treats tokens inside code as cites', () => {
    ...
    expect(parseMentions(body)).toEqual({ members: ['ramnique', 'harsh'], here: true, mythril: false });
    expect(parseMentions('a bare @ramnique and @here are prose')).toEqual({ members: [], here: false, mythril: false });
    expect(parseMentions('[@mythril](#mythril) summarise')).toMatchObject({ mythril: true });
  });
  ...
  it('a space token refers, never addresses: it stamps nothing, relabels from the listing, and flattens to #Name', () => {
    ...
    expect(parseMentions(body)).toEqual({ members: ['harsh'], here: false, mythril: false });
    ...
    expect(parseMentions('[#x](#member:harsh) [@y](#space:S1)')).toEqual({ members: [], here: false, mythril: false });
  ...
  it('the backfill rewrites the pre-token spelling for known ids only, punctuation intact, idempotently', () => {
    ...
    const legacy = 'cc @ramnique, @harsh. and @nobody (@here) `@harsh` @mythril';
    const once = legacyToTokens(legacy, names);
    expect(once).toBe(`cc ${tok('ramnique', 'Ramnique')}, ${tok('harsh', 'Harsh')}. and @nobody ([@here](#here)) \`@harsh\` [@mythril](#mythril)`);
...
describe.each([['memory'], ['postgres']] as const)('mentions (%s store)', (storeKind) => {
  ...
  it('stamps only tokens naming space members; a bare name or a non-member id is prose', async () => {
    ...
    expect(root).toMatchObject({ mentions: ['ramnique'], mentionsHere: false, mentionsMythril: false });
