...
import type { ChangeSet, CreateAssetResult, MoveAssetResult, ReadAssetResult } from '@mythril/spaces-protocol';
...
describe.each([['memory'], ['postgres']] as const)('asset move/delete/restore (%s store)', (storeKind) => {
  ...
  it('agents create, move and delete over MCP by id, attributed with their name', async () => {
    const agent = await agentClient(harbor, 'dev-ramnique', { agentName: 'Mythril' });
    ...
    expect(born.changeSet.attribution).toEqual({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
    const listing = await ramnique.get(`/v1/spaces/${spaceId}/assets`);
    ...
    if (moved.outcome === 'moved') {
      expect(moved.changeSet.attribution).toEqual({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
    ...
    expect(deleted.changeSet?.attribution).toEqual({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
    await agent.close();
  });
});
