...
import { mcpTools } from '@mythril/spaces-protocol';
...
// (Mythril's included — no privileged path) uses.
...
describe('agent face (MCP)', () => {
  ...
  it('create_asset blob variant files an already-uploaded attachment — no byte movement', async () => {
    ...
    const client = await mcpClient('dev-harsh', { 'x-agent-name': 'Mythril' });
  ...
  it('x-acting-mode: scheduled attributes automations honestly', async () => {
    const client = await mcpClient('dev-harsh', { 'x-acting-mode': 'scheduled', 'x-agent-name': 'Mythril' });
    const read = (await client.callTool({ name: 'read_asset', arguments: { spaceId, assetId: roadmapId } }))
      .structuredContent as { content: string; version: number };
    const propose = (
      await client.callTool({
    ...
    expect(propose.changeSet.attribution.agentName).toBe('Mythril');
    await client.close();
  });
