...
import type { ServerFrame, Space } from '@mythril/spaces-protocol';
...
describe.each([['memory'], ['postgres']] as const)('direct messages (%s store)', (storeKind) => {
  ...
  it('the agent face sees DMs only when asked, with kind and participants, and works on them like any space', async () => {
    const agent: Client = await agentClient(harbor, 'dev-ramnique', { agentName: 'Mythril' });
    ...
    try {
      ...
      expect(mine.author).toMatchObject({ memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' });
    } finally {
      await agent.close();
    }
  });
