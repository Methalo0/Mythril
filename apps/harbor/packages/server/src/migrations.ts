...
export const MIGRATIONS: Migration[] = [
  ...
  },
  {
    ...
    statements: [
      ...
      // Thread parentage: the "<!-- mythril:topic parent=msg:<id> … -->" marker
      ...
      `update topics set anchor_message_id = claims.parent from (
        select distinct on (parent) topic_id, parent from (
          select distinct on (topic_id) topic_id, posted_at,
            substring(body from '<!--\\s*mythril:(?:topic|thread)\\s+parent=msg:([0-9A-Za-z_-]+)') as parent
          from messages
  ...
  },
  {
    ...
    statements: [
      ...
      `delete from messages where thread_root is not null
        and body ~ '<!--\\s*mythril:(topic|thread)\\s+parent=msg:'`,
  ...
  },
  {
    ...
    statements: [
      ...
      `alter table messages add column if not exists mentions_mythril boolean not null default false`,
