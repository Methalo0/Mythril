import type { ActivityItem, ActivityKind, ActivityPage, ServerFrame } from '@mythril/spaces-protocol';
...
function stampFields(stamps: MentionStamps): Pick<Message, 'mentions' | 'mentionsHere' | 'mentionsMythril'> {
  return { mentions: [...stamps.members], mentionsHere: stamps.here, mentionsMythril: stamps.mythril };
}

function stampsOf(message: Message): MentionStamps {
  return { members: message.mentions, here: message.mentionsHere, mythril: message.mentionsMythril };
}
...
export class HarborService {
  ...
  private async stampsFor(spaceId: string, body: string): Promise<MentionStamps> {
    ...
    return { members, here: parsed.here, mythril: parsed.mythril };
  }
   ...
   * spelling — "@<memberId>", bare "@here" / "@mythril" — becomes mention
  ...
  async deleteMessage(
    ...
    return this.locked(spaceId, async () => {
      ...
      return this.foldLive(spaceId, { ...rest, body: '', deletedAt: at, mentions: [], mentionsHere: false, mentionsMythril: false });
    });
  }
