import type { ActivityKind } from '@mythril/spaces-protocol';
...
export class MemoryStore implements Store {
  ...
  private stampFields(stamps: MentionStamps): Pick<Message, 'mentions' | 'mentionsHere' | 'mentionsMythril'> {
  ...
  async markMessageDeleted(spaceId: string, messageId: string, deletedAt: string): Promise<void> {
    ...
    if (existing) {
      ...
      this.replace(s, { ...rest, body: '', deletedAt, mentions: [], mentionsHere: false, mentionsMythril: false });
    }
    ...
    for (let i = 0; i < s.events.length; i++) {
      ...
      if (e.event.type === 'message' && e.event.message.id === messageId) {
        ...
        s.events[i] = { ...e, event: { type: 'message', message: { ...rest, body: '', deletedAt, mentions: [], mentionsHere: false, mentionsMythril: false } } };
        break;
      }
    }
  }
