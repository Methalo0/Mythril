import type { ActivityKind, Attribution } from '@mythril/spaces-protocol';
...
function rowToMessage(r: MessageRow): Message {
  return {
    ...
    mentionsMythril: r.mentions_mythril ?? false,
  };
}
...
export class PgStore implements Store {
  ...
  async appendMessage(message: Message): Promise<void> {
    await this.sql.query(
      `insert into messages (id, space_id, thread_root, author, body, posted_at, stream_offset, reply_count, last_reply_at, anchor_change_set_id, poll, mentions, mentions_here, mentions_mythril, search_text)
  ...
  async markMessageEdited(spaceId: string, messageId: string, body: string, editedAt: string, stamps: MentionStamps): Promise<void> {
    await this.sql.query(
      `update messages set body = $3, edited_at = $4, mentions = $5::jsonb, mentions_here = $6, mentions_mythril = $7, search_text = $8
       where space_id = $1 and id = $2`,
      [spaceId, messageId, body, editedAt, JSON.stringify(stamps.members), stamps.here, stamps.mythril, searchTextFor(body)],
    );
    ...
    await this.sql.query(
      `update events set event = jsonb_set(event, '{message}', (event->'message') || jsonb_build_object(
         'body', $3::text, 'editedAt', $4::text, 'mentions', $5::jsonb, 'mentionsHere', $6::boolean, 'mentionsMythril', $7::boolean))
       where space_id = $1 and event->>'type' = 'message' and event->'message'->>'id' = $2`,
      [spaceId, messageId, body, editedAt, JSON.stringify(stamps.members), stamps.here, stamps.mythril],
    );
  }

  async restampMessage(spaceId: string, messageId: string, stamps: MentionStamps): Promise<void> {
    await this.sql.query(
      `update messages set mentions = $3::jsonb, mentions_here = $4, mentions_mythril = $5 where space_id = $1 and id = $2`,
      [spaceId, messageId, JSON.stringify(stamps.members), stamps.here, stamps.mythril],
    );
    await this.sql.query(
      `update events set event = jsonb_set(event, '{message}', (event->'message') || jsonb_build_object(
         'mentions', $3::jsonb, 'mentionsHere', $4::boolean, 'mentionsMythril', $5::boolean))
       where space_id = $1 and event->>'type' = 'message' and event->'message'->>'id' = $2`,
      [spaceId, messageId, JSON.stringify(stamps.members), stamps.here, stamps.mythril],
    );
  }
  ...
  async markMessageDeleted(spaceId: string, messageId: string, deletedAt: string): Promise<void> {
    await this.sql.query(
      `update messages set body = '', deleted_at = $3, poll = null, mentions = '[]'::jsonb, mentions_here = false, mentions_mythril = false, search_text = ''
       where space_id = $1 and id = $2`,
    ...
    await this.sql.query(
      `update events set event = jsonb_set(event, '{message}', ((event->'message') #- '{poll}') || jsonb_build_object(
         'body', '', 'deletedAt', $3::text, 'mentions', '[]'::jsonb, 'mentionsHere', false, 'mentionsMythril', false))
       where space_id = $1 and event->>'type' = 'message' and event->'message'->>'id' = $2`,
      [spaceId, messageId, deletedAt],
    );
  }
