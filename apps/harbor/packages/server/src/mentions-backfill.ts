import { mentionToken } from '@mythril/spaces-protocol';
...
// "@<memberId>" as bare text and "@here" / "@mythril" as bare words; readers
  ...
  return text
    ...
    .map((part, i) => {
      ...
      return part.replace(LEGACY_RE, (raw: string, pre: string, handle: string) => {
        ...
        if (lower === 'mythril') return `${pre}${mentionToken({ kind: 'mythril' })}${tail}`;
