...
import { routes } from '@mythril/spaces-protocol';
...
export function buildHttpApp(deps: {
  ...
  // hands the invite into the app as mythril://open?type=spaces&org=…&invite=…
  ...
  app.get('/join/:token', async (c) => {
    ...
    const deep = `mythril://open?type=spaces&org=${encodeURIComponent(service.org.address)}&invite=${encodeURIComponent(token)}`;
  ...
  // handed into the app as a mythril:// deep link, the org named by its
  ...
  const landing = (target: URLSearchParams) => {
    ...
    const deep = `mythril://open?type=spaces&${target.toString()}`;
    return `<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Open in Mythril</title>` +
      ...
      `<main><p>This link opens in Mythril.</p><a class="b" href="${deep}">Open in Mythril</a></main>` +
...
const DOWNLOAD_URL = 'https://github.com/mythrillabs/mythril/releases/latest';
...
function invitePage(page: InvitePage): string {
  ...
  return (
    ...
    `<a class="b" href="${page.deep}">Open in Mythril</a>` +
    `<p class="s">Don't have Mythril? <a href="${DOWNLOAD_URL}">Download it</a>, then open this link again.</p></main>` +
