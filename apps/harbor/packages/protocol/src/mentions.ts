...
export function noMentions(): MentionStamps {
  return { members: [], here: false, mythril: false };
}
 ...
 * `[@…](#member:…)` / `[@here](#here)` / `[@mythril](#mythril)` address;
 * `[#…](#space:…)` refers.
 */
export const MENTION_TOKEN_RE = /\[@([^\]\n]*)\]\(#(?:member:([^)\s]+)|(here)|(mythril))\)|\[#([^\]\n]*)\]\(#space:([^)\s]+)\)/g;
...
function refOf(label: string, id: string | undefined, here: string | undefined, spaceLabel: string | undefined, spaceId: string | undefined): MentionRef {
  ...
  return here !== undefined ? { kind: 'here' } : { kind: 'mythril' };
}
...
export function mapMentionTokens(body: string, fn: (ref: MentionRef, raw: string) => string): string {
  return body
    ...
    .map((part, i) => {
      ...
      return part.replace(MENTION_TOKEN_RE, (raw: string, label: string, id?: string, here?: string, _mythril?: string, spaceLabel?: string, spaceId?: string) =>
...
export function parseMentions(body: string): MentionStamps {
  ...
  mapMentionTokens(body, (ref, raw) => {
    ...
    else if (ref.kind === 'mythril') mythril = true;
    return raw;
  });
  return { members, here, mythril };
}

/** True when the body deliberately addresses @mythril (a token, never the word). */
export function addressesMythril(body: string): boolean {
  return parseMentions(body).mythril;
}
...
export const MENTION_GRAMMAR =
  ...
  '`[@here](#here)` addresses everyone in the space and `[@mythril](#mythril)` addresses the ' +
