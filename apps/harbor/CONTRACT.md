...
> The wire-level contract between Harbor (the spaces server) and everything that talks to it — the Mythril app, any MCP agent, and the stub server. The contract **is** the `@mythril/spaces-protocol` package in this workspace; this document is its narrative. The product/protocol spec it implements lives in [mythrillabs/harbor](https://github.com/mythrillabs/harbor) (`SPACES_SPEC.md`) — spec §references below point there.
...
- `apps/x` packages depend on it in-repo: `"@mythril/spaces-protocol": "file:../../harbor/packages/protocol"` (adjust relative path per package). Run its `build` before consuming (`cd apps/harbor && pnpm install && pnpm build`).
- The stub Harbor (`packages/server`, `@mythril/harbor`) consumes it as `workspace:*`; the real Harbor grows in that same package behind the same contract.
...
- **One core, three doors.** `service.ts` is the single implementation; `http.ts`
  ...
  projections. Mythril's agent gets no privileged path — enforced by there being
  no other door.
