...
// renders under). The other __mythril_* sentinel tab paths died with the tab
// strip — sections are plain view state now.
const BASES_DEFAULT_TAB_PATH = '__mythril_bases_default__'
 ...
 *
 * - mythril:  mythril/2026-03-20/meeting-xxx.md  → keeps date folders as-is
...
/**
 * Parse a mythril:// deep link into a ViewState. Returns null if the URL is
 * malformed or names an unknown target.
 *
 * Shape: mythril://open?type=<file|chat|graph|task|suggested-topics|meetings|live-notes|email>&...
...
function ContentHeader({
  ...
  return (
    <header
      className={cn("mythril-titlebar titlebar-drag-region flex h-10 shrink-0 items-stretch border-b border-border bg-background overflow-hidden", className)}
      style={{
...
function App() {
  ...
  const refreshVoiceAvailability = useCallback(() => {
    const probe = Promise.all([
      ...
      const mythrilConnected = oauthState.config?.mythril?.connected ?? false
      const hasVoice = !!config.deepgram || mythrilConnected
      const hasTts = !!config.elevenlabs || mythrilConnected
  ...
  // after the user signs in to Mythril (so we catch users who weren't signed
  ...
  useEffect(() => {
    ...
    const cleanup = window.ipc.on('oauth:didConnect', (event) => {
      if (event.provider === 'mythril' && event.success) {
        void run()
      }
    })
    return cleanup
  ...
  // Mythril sign-in (sign-in seeds the initial selection main-side and marks
  ...
  useEffect(() => {
    ...
    window.addEventListener('mythril:permission-needed', onNeeded)
    return () => window.removeEventListener('mythril:permission-needed', onNeeded)
  }, [])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      playPopCue()
      toast('Ask Mythril from anywhere', {
  ...
  // Quick-ask "Open in Mythril": the ONE deliberate bridge between the
  ...
  const [activeChatTabId, setActiveChatTabId] = useState(() => {
    const saved = readAssistantPreference('mythril-assistant-active-tab')
  ...
  useEffect(() => {
    ...
    writeAssistantPreference('mythril-assistant-active-tab', activeChatTabId)
  ...
  if (!draftsRestoredRef.current) {
    ...
    for (const tab of chatTabs) {
      const draft = readAssistantPreference(`mythril-assistant-draft:${tab.chatId}`)
  ...
  const selectionByTabRef = useRef(new Map<string, { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'ultrahigh' }>())
  ...
  const setChatDraftForTab = useCallback((tabId: string, text: string) => {
    ...
    writeAssistantPreference(`mythril-assistant-draft:${chatId}`, text || null)
  }, [chatIdForTab])
  ...
  // The pending "Mythril now recommends…" prompt, if any (see the modal).
  ...
  const handleHomeComposerSubmit = useCallback((
    ...
    if (target) {
      ...
      if (target.kind === 'comment') {
        ...
        void window.ipc.invoke('todo:addSubItem', { parentKey: target.parentKey, text, run: /(^|\s)@mythril\b/i.test(text), attachments, model, permissionMode })
      } else {
        // A picked code lane is delegation intent as explicit as @mythril —
        ...
        const code = codeProject
          ...
          : undefined
        void window.ipc.invoke('todo:addItem', { text, run: /(^|\s)@mythril\b/i.test(text) || !!code, attachments, model, permissionMode, code })
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener('mythril:open-copilot-edit-live-note', handler as EventListener)
    return () => window.removeEventListener('mythril:open-copilot-edit-live-note', handler as EventListener)
  }, [submitFromPalette])
  ...
  useEffect(() => {
    ...
    window.addEventListener('mythril:open-live-note-panel', handler as EventListener)
    return () => window.removeEventListener('mythril:open-live-note-panel', handler as EventListener)
  }, [])
  ...
  useEffect(() => {
    ...
    window.addEventListener('mythril:open-copilot-prompt', handler as EventListener)
    return () => window.removeEventListener('mythril:open-copilot-prompt', handler as EventListener)
  }, [submitFromPalette])
  ...
  useEffect(() => {
    void window.ipc.invoke('app:consumeUpdateInfo', null).then(({ version, updatedFrom }) => {
      ...
      toast(`Updated to v${version}`, {
        description: `Mythril was updated from v${updatedFrom}.`,
        action: {
          label: "What's new",
          onClick: () => window.open(`https://github.com/Methal0/Mythril/releases/tag/v${version}`, '_blank'),
        },
        duration: 10000,
        closeButton: true,
      })
  ...
  const notifyVoiceUnavailable = useCallback((reason: 'voice' | 'failed') => {
    ...
    toast('Hover mode needs voice', {
      description:
        'Sign in to Mythril — or add your own Deepgram and ElevenLabs keys — to talk to your Skipper.',
      duration: 8000,
  ...
  // GET /_mythril/app and get live changes via the SSE theme event.
  useEffect(() => {
    const report = () => {
  ...
  const applyAppNavigation = useCallback((result: Record<string, unknown>) => {
    ...
    // and if the user is in another app, bring Mythril forward.
  ...
  useEffect(() => {
    ...
    for (const tool of completed) {
      ...
      if (result && result.success && typeof result.workspaceRelPath === 'string') {
        ...
        window.dispatchEvent(new CustomEvent('mythril:deck-touched', { detail: { path: result.workspaceRelPath } }))
      }
    }
  ...
  useEffect(() => {
    ...
    for (const tool of completed) {
      ...
      if (result && result.success && typeof result.workspaceRelPath === 'string') {
        ...
        window.dispatchEvent(new CustomEvent('mythril:spreadsheet-touched', { detail: { path: result.workspaceRelPath } }))
      }
    }
  }, [sessionChat.chatState?.conversation, runId, navigateToView])
  ...
  // Keyboard shortcut: Ctrl+Shift+K opens a new chat tab (Mythril).
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && !e.altKey && e.key.toLowerCase() === 'k') {
        e.preventDefault()
  ...
  const handleOnboardingComplete = useCallback(async (opts?: { startTour?: boolean }) => {
    ...
    // Mythril: product tour removed — onboarding goes straight to the app.
    void opts
  }, [])
        ...
        try {
          ...
          if (fileContent && fileContent.trim()) {
            ...
            if (notes) {
              ...
              await handleVoiceNoteCreated(notePath)
              // Notes are done — bring Mythril to the foreground on the
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <SidebarSectionProvider defaultSection="tasks" onSectionChange={(section) => {
        ...
        <div className="mythril-shell flex h-svh w-full overflow-hidden">
...
{/* Middle pane. Section views wrapped in <Activity> stay
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          placeholder={homeComposeTarget
                            ? (homeComposeTarget.kind === 'sub'
                                ? 'Add a step… @mythril hands it off'
                                : homeComposeTarget.kind === 'comment'
                                  ? 'Tell @mythril what to change…'
                                  : homeComposeTarget.kind === 'chatReply'
                                    ? 'Reply…'
                                    : 'Add a to-do… @mythril hands it off')
              ...
              {sectionMounted('workspace') && (
                ...
                <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                  <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-3 p-8 text-center">
                    ...
                    <p className="max-w-sm text-sm text-muted-foreground">{selectedProject ? 'Choose a chat or file in the rail, or start a new conversation with Mythril.' : 'Choose a project in the rail, or create one to organize your chats and local files.'}</p>
          ...
          <DialogHeader>
            ...
            <DialogDescription className="pt-1 leading-relaxed">
              {retentionNotice?.chatDays != null
                ? `To save disk space, Mythril now deletes chats that have been inactive for ${retentionNotice.chatDays}+ days, along with old background-task transcripts.`
                : 'To save disk space, Mythril now deletes old background-task transcripts.'}
      ...
      <ComposioGoogleMigrationModal
        ...
        onReconnect={() => {
          // Trigger the mythril-mode Google connect flow. With no credentials
          // and the user signed in to Mythril, the main process opens the
          // webapp `/oauth/google/start` URL. The deep link returns and
          // completeMythrilGoogleConnect persists the tokens.
      ...
      <Dialog open={showMeetingPermissions} onOpenChange={setShowMeetingPermissions}>
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            ...
            <DialogDescription>
              Mythril needs <strong>Screen Recording</strong> permission to capture meeting audio from other apps (Zoom, Meet, etc.). This feature won't work without it.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            ...
            <ol className="list-decimal list-inside space-y-1.5">
              ...
              <li>Toggle on <strong>Mythril</strong></li>
