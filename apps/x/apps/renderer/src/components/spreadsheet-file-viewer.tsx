...
export function SpreadsheetFileViewer({ path }: SpreadsheetFileViewerProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('mythril:spreadsheet-touched', onTouched)
    return () => window.removeEventListener('mythril:spreadsheet-touched', onTouched)
  }, [path, refetch])
  ...
  return (
    <div className="flex h-full w-full flex-col bg-background">
      <div className="mythril-header flex shrink-0 items-center gap-2 border-b border-border px-4">
        ...
        {findOpen ? (
          <div className="flex animate-in items-center gap-1 rounded-md border border-transparent bg-[var(--mythril-wash)] px-1.5 py-0.5 focus-within:border-border fade-in slide-in-from-right-2 duration-200">
      ...
      {selection && (
        <div className="flex animate-in items-start gap-2 border-b border-border px-4 py-1 text-xs fade-in slide-in-from-top-1 duration-200">
          ...
          >
            {copied
              ? <CheckIcon className="size-3.5 animate-in text-[var(--mythril-success)] zoom-in-50 duration-200" />
        ...
        {data.totalRows > 0 && (
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              placeholder="#"
              className="w-14 rounded border border-transparent bg-[var(--mythril-wash)] px-1.5 py-0.5 text-xs text-foreground outline-none transition-colors focus:border-border"
            />
          </div>
        )}
        {totalPages > 1 && (
