...
/**
 * The Mythril mascot as an animated inline SVG: a round pale character sitting
 * in a wooden mythril holding an oar. The mouth is driven every animation
...
export function TalkingHeadOverlay({ ttsState, getLevel, onDismiss }: TalkingHeadOverlayProps) {
  ...
  return (
    <div
      ...
      role="img"
      aria-label="Mythril talking head"
    >
      <style>{`
...
export function MascotFaceIcon({ className, size = 16, style }: { className?: string; size?: number | string; style?: React.CSSProperties }) {
  // The Mythril mark as a 24px outline glyph (from mythril-assistant-outline.svg),
