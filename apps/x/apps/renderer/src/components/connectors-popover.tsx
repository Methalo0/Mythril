...
export function ConnectorsPopover({ children, tooltip, open: openProp, onOpenChange, mode = "all" }: ConnectorsPopoverProps) {
        ...
        <div className="shrink-0">
          {state.isLoading ? (
            ...
            <Button
              ...
              className="h-7 px-2 text-xs"
            >
              {provider === 'mythril' ? 'Log Out' : 'Disconnect'}
            </Button>
          ) : (
            <Button
              variant="default"
            ...
            >
              {state.isConnecting ? (
                ...
                provider === 'mythril' ? 'Log In' : 'Connect'
              )}
            </Button>
          )}
        </div>
  ...
  const isMythrilUnconnected = (() => {
    if (!c.providers.includes('mythril')) return false
    const mythrilState = c.providerStates['mythril']
    return !mythrilState?.isConnected || mythrilState?.isLoading
  })()

  const allConnected = isUnconnectedMode && !isMythrilUnconnected && !hasUnconnectedEmailCalendar && !hasUnconnectedMeetingNotes
      ...
      >
        ...
        <div className="p-2">
          {c.providersLoading ? (
            ...
            <>
              {/* Mythril Account - show in "all" mode always, or in "unconnected" mode only when not connected */}
              {c.providers.includes('mythril') && (() => {
                const mythrilState = c.providerStates['mythril']
                const isMythrilConnected = mythrilState?.isConnected && !mythrilState?.isLoading
                if (isUnconnectedMode && isMythrilConnected) return null
                return (
                  <>
                    <div className="px-2 py-1.5">
                      <span className="text-xs font-medium text-muted-foreground">Account</span>
                    </div>
                    {renderOAuthProvider('mythril', 'Mythril', <User className="size-4" />, 'Log in to your Mythril account')}
