...
// the sessions the @mythril mention machinery owns, one per space thread.
...
export function ChatHistoryView({
  ...
  const subtitle = (() => {
    if (activeFacet) {
      ...
      return `${plural(activeFacet.count, 'thread', 'threads')} where Mythril was mentioned in ${where}${org}, newest first.`
    }
    if (showMentions) {
      return `${plural(threadRuns.length, 'thread', 'threads')} where Mythril was mentioned across your spaces, newest first.`
    }
  ...
  const emptyMessage = activeFacet
    ...
    : hasThreads && !showMentions
      ? 'No assistant chats yet. The Space chats tab lists the conversations Mythril had inside your spaces.'
      : 'No chats yet.'
                        ...
                        {renamingId === run.id ? (
                          <div className="flex items-center px-4 py-1.5">
                            <input
                              ...
                              className="h-7 min-w-0 flex-1 rounded-md border border-transparent bg-[var(--mythril-wash)] px-2 text-sm outline-none focus:border-border focus:ring-1 focus:ring-ring"
                            />
                          </div>
                        ) : (
                          <>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {runs.find((r) => r.id === pendingDeleteId)?.origin?.kind === 'space_thread'
                ? 'Are you sure you want to delete this thread’s conversation? The next @mythril mention in the thread starts a fresh one.'
