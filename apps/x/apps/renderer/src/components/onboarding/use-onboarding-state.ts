...
export type OnboardingPath = 'mythril' | 'byok' | null
...
export function useOnboardingState(open: boolean, onComplete: (opts?: { startTour?: boolean }) => void) {
  ...
  // New step flow:
  // Mythril path: 0 (welcome) → 2 (connect) → 3 (code mode) → 4 (done)
  ...
  // Auto-advance from Mythril sign-in step when OAuth completes
  useEffect(() => {
    if (onboardingPath !== 'mythril' || currentStep !== 0) return
