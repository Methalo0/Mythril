...
export function AppFrame({ app, onBack }: { app: mythrilApp.AppSummary; onBack: () => void }) {
        ...
        {app.kind === 'local' && (
          app.publish ? (
            <button
              ...
              className="flex items-center gap-1.5 rounded-md px-2 py-1.5 text-sm font-medium text-[var(--mythril-success)] hover:bg-[var(--mythril-success)]/10"
            >
          ...
          {loadState === 'stuck' && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-background/95 text-sm">
              <div className="text-muted-foreground">
                {serverDown ? 'The Mythril apps server is still starting up.' : 'This app is taking too long to load.'}
              </div>
              <button
                type="button"
                onClick={() => setReloadNonce((n) => n + 1)}
