...
const UPDATE: RecommendationUpdate = {
  ...
  rows: [
    ...
    { slot: 'knowledgeGraph', current: null, recommended: { provider: 'mythril', model: 'Auto-background' } },
    { slot: 'chatTitle', current: { provider: 'mythril', model: 'lite' }, recommended: null },
  ],
}
...
describe('ModelRecommendationUpdateModal', () => {
  ...
  it('applies exactly the checked slots and closes', async () => {
    ...
    expect(invokes).toEqual([{
      ...
      args: { flavor: 'mythril', hash: 'abc', apply: ['assistantModel', 'chatTitle'] },
    }])
  })
  ...
  it('"Not Now" answers with no slots', async () => {
    ...
    expect(invokes).toEqual([{
      channel: 'models:resolveRecommendationUpdate',
      args: { flavor: 'mythril', hash: 'abc', apply: [] },
    }])
  })
