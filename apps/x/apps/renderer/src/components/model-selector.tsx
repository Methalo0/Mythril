...
export type ReasoningEffortLevel = 'low' | 'medium' | 'high' | 'xhigh' | 'ultrahigh'
...
const REASONING_EFFORT_OPTIONS: Array<{ value: '' | ReasoningEffortLevel; label: string; short: string; hint: string }> = [
  ...
  { value: 'low', label: 'Low', short: 'Low', hint: 'Minimal thinking' },
  { value: 'medium', label: 'Normal', short: 'Norm', hint: 'Moderate thinking' },
  { value: 'high', label: 'High', short: 'High', hint: 'Deep thinking, costs more' },
  { value: 'xhigh', label: 'X-High', short: 'XHi', hint: 'Very deep thinking' },
  { value: 'ultrahigh', label: 'Ultrahigh', short: 'Ultra', hint: 'Maximum thinking budget' },
]
...
export function ModelSelector({
  ...
  // provider name — typing "mythril" surfaces the whole Mythril group.
  ...
  const renderEffortSubPanel = () => {
    ...
    return createPortal(
      <div
        ...
        className="z-50 rounded-2xl border-none bg-popover p-2 text-popover-foreground shadow-[var(--mythril-shadow)]"
        onMouseEnter={() => {
          graceRef.current = null
          cancelSubClose()
        }}
