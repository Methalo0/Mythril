...
import { MythrilMentionPopover } from './mythril-mention-popover'
import '@/styles/editor.css'

type MythrilMentionMatch = {
  range: { from: number; to: number }
}

type MythrilBlockEdit = {
...
export const MarkdownEditor = forwardRef<MarkdownEditorHandle, MarkdownEditorProps>(function MarkdownEditor({
  ...
  const [activeMythrilMention, setActiveMythrilMention] = useState<MythrilMentionMatch | null>(null)
  const [mythrilBlockEdit, setMythrilBlockEdit] = useState<MythrilBlockEdit | null>(null)
  const [mythrilAnchorTop, setMythrilAnchorTop] = useState<{ top: number; left: number; width: number } | null>(null)
  const mythrilBlockEditRef = useRef<MythrilBlockEdit | null>(null)
  ...
  const updateMythrilMentionState = useCallback(() => {
    if (!editor) return
    const { selection } = editor.state
    if (!selection.empty) {
      setActiveMythrilMention(null)
      setMythrilAnchorTop(null)
      return
    }
    ...
    // Match @mythril at a word boundary (preceded by nothing or whitespace)
    const match = textBefore.match(/(^|\s)@mythril$/)
    if (!match) {
      setActiveMythrilMention(null)
      setMythrilAnchorTop(null)
      return
    ...
    const triggerStart = textBefore.length - '@mythril'.length
    const from = selection.from - (textBefore.length - triggerStart)
    const to = selection.from
    setActiveMythrilMention({ range: { from, to } })
  ...
  const updateAtMentionState = useCallback(() => {
    ...
    // If the full "@mythril" is already typed, let updateMythrilMentionState handle it
    if (query === 'mythril') {
      setActiveAtMention(null)
      setAtAnchorPosition(null)
      return
  ...
  useEffect(() => {
    if (!editor) return
    editor.on('update', updateMythrilMentionState)
    editor.on('selectionUpdate', updateMythrilMentionState)
    return () => {
      editor.off('update', updateMythrilMentionState)
      editor.off('selectionUpdate', updateMythrilMentionState)
    }
  }, [editor, updateMythrilMentionState])
  ...
  // When a tell-mythril block is clicked, compute anchor and open popover
  useEffect(() => {
    if (!mythrilBlockEdit || !editor) return
    const wrapper = wrapperRef.current
    if (!wrapper) return
    const coords = editor.view.coordsAtPos(mythrilBlockEdit.nodePos)
    const wrapperRect = wrapper.getBoundingClientRect()
    const proseMirrorEl = wrapper.querySelector('.ProseMirror') as HTMLElement | null
    const pmRect = proseMirrorEl?.getBoundingClientRect()
    setMythrilAnchorTop({
  ...
  const handleMythrilAdd = useCallback(async (instruction: string) => {
    if (!editor) return

    if (mythrilBlockEdit) {
      // Editing existing taskBlock — update its data attribute
      const { nodePos } = mythrilBlockEdit
      ...
      setMythrilBlockEdit(null)
      mythrilBlockEditRef.current = null
      setMythrilAnchorTop(null)
      return
    }

    if (activeMythrilMention) {
      // Insert a temporary processing block
      const blockData: Record<string, unknown> = { instruction, processing: true }

      const insertFrom = activeMythrilMention.range.from
      const insertTo = activeMythrilMention.range.to
      ...
      try {
          ...
          if (notePath) {
            setTimeout(async () => {
              try {
                ...
                console.error('[MythrilAdd] Failed to write target tags:', err)
              }
            }, 500)
          }
        } else {
        ...
        console.error('[MythrilAdd] Processing failed:', error)
  ...
  }, [editor, activeMythrilMention, mythrilBlockEdit, frontmatter, onFrontmatterChange, notePath])

  const handleMythrilRemove = useCallback(() => {
    if (!editor || !mythrilBlockEdit) return
    const { nodePos } = mythrilBlockEdit
    const node = editor.state.doc.nodeAt(nodePos)
    if (node) {
      editor
  ...
  const atMentionOptions = useMemo(() => [
    { value: 'mythril', label: '@mythril', description: 'Research, schedule, or run tasks with AI' },
  ], [])
  ...
  const handleSelectAtMention = useCallback((value: string) => {
    ...
    if (value === 'mythril') {
      // Replace "@<partial>" with "@mythril" — this triggers updateMythrilMentionState
      editor
        .chain()
        .focus()
        .insertContentAt(
    ...
    <div className="tiptap-editor" onKeyDown={handleKeyDown}>
      <EditorToolbar
        ...
        onOpenLiveNote={notePath ? () => {
          window.dispatchEvent(new CustomEvent('mythril:open-live-note-panel', {
      ...
      <div className="editor-content-wrapper" ref={wrapperRef} onScroll={handleScroll}>
        ...
        <MythrilMentionPopover
          open={Boolean((activeMythrilMention || mythrilBlockEdit) && mythrilAnchorTop)}
          anchor={mythrilAnchorTop}
          initialText={mythrilBlockEdit?.existingText ?? ''}
          onAdd={handleMythrilAdd}
          onRemove={mythrilBlockEdit ? handleMythrilRemove : undefined}
          onClose={() => {
            setActiveMythrilMention(null)
            setMythrilBlockEdit(null)
            mythrilBlockEditRef.current = null
