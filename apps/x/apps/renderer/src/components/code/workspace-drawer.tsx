...
export function CodeWorkspaceDrawer({
  ...
  return (
    ...
    >
      ...
      {panel === 'changes' && (
        <div className="mythril-header flex shrink-0 items-center gap-1.5 border-b border-border px-3 text-xs text-muted-foreground">
          {gitStatus?.isRepo ? (
            <>
                ...
                return (
                  ...
                  >
                    <Icon className={cn(
                      'size-3.5 shrink-0',
                      file.state === 'deleted'
                        ? 'text-[var(--mythril-attention)]'
                        : file.state === 'modified' || file.state === 'renamed'
                          ? 'text-muted-foreground'
                          : 'text-[var(--mythril-success)]',
                    )} />
                    <span className="min-w-0 flex-1 truncate font-mono">{file.path}</span>
                    <span className="flex shrink-0 gap-1.5 tabular-nums">
                      {file.insertions !== null && <span className="text-[var(--mythril-success)]">+{file.insertions}</span>}
                      {file.deletions !== null && <span className="text-[var(--mythril-attention)]">−{file.deletions}</span>}
                    </span>
                  </button>
                )
              })}
