...
export function GoogleDocPickerDialog({
  ...
  // the Mythril web session. So the only desktop prerequisite is being signed
  // in to Mythril — it needs NO prior Google connection and NO drive.file scope
  ...
  const loadStatus = useCallback(async () => {
    try {
      const account = await window.ipc.invoke('account:getMythril', null)
      setSignedIn(account.signedIn)
      setError(null)
    } catch (err) {
      setSignedIn(null)
      setError(err instanceof Error ? err.message : 'Failed to check your Mythril sign-in')
    }
  }, [])
  ...
  const handleChoose = useCallback(async () => {
    ...
    // Managed pick: the Mythril backend runs the whole grant + pick in the
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          {signedIn === null && error ? (
            ...
            <div className="flex min-h-[280px] flex-1 items-center justify-center text-sm text-muted-foreground">
              ...
              Checking your Mythril sign-in…
            </div>
          ) : !signedIn ? (
            <div className="flex min-h-[300px] flex-1 flex-col items-center justify-center gap-4 px-8 py-8 text-center">
              <div className="max-w-sm text-sm text-muted-foreground">
                Sign in to Mythril to add Google Docs from Drive. The picker uses your
                Mythril account — no Google credentials or API key needed.
              </div>
              <Button variant="outline" onClick={() => void loadStatus()}>
                <RefreshCw className="size-4" />
