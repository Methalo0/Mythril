...
function planMarker(status?: string) {
  if (status === 'completed') return <CheckCircle2 className="size-3.5 shrink-0 text-[var(--mythril-success)]" />
  if (status === 'in_progress') return <CircleDot className="size-3.5 shrink-0 text-[var(--mythril-git)]" />
  ...
  return (
    <div className="flex flex-col gap-2 py-1">
      {rows.map((row) => {
            ...
            <div key={row.id} className="flex flex-col gap-1">
              <div className="flex items-center gap-2 text-sm">
                {running ? (
                  ...
                  <CheckCircle2 className="size-3.5 shrink-0 text-[var(--mythril-success)]" />
                )}
        ...
        return (
          <div key={row.id} className={cn('flex items-center gap-2 text-xs', denied ? 'text-red-600' : 'text-[var(--mythril-success)]')}>
            {denied ? '✕' : '✓'}
