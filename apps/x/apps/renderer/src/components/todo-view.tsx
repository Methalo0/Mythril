...
// ---------------------------------------------------------------------------
// The home to-do list — one rolling ~/.mythril/todo.md shared with @mythril.
...
// (working spinners) from todo:events. Tagging @mythril in a line delegates
...
type TodoViewProps = {
  ...
  getRunModel?: () => { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'ultrahigh' } | undefined
...
const MYTHRIL_MENTION_RE = /(^|\s)@mythril\b/i
...
function mentionsMythril(text: string): boolean {
  return MYTHRIL_MENTION_RE.test(text)
}
...
// ---------------------------------------------------------------------------
// @mythril autocomplete — shared by every composer (main, reply, sub-task).
...
function useMention(text: string, setText: (t: string) => void) {
  ...
  const show = match !== null && !/(^|\s)@mythril$/i.test(text)
  const complete = () => {
    if (!match) return
    setText(`${text.slice(0, match.index)}${match[1]}@mythril `)
  }
...
function MentionPopup({ onPick }: { onPick: () => void }) {
  return (
    <button
      ...
      className="absolute bottom-full left-3 z-10 mb-1.5 flex items-center gap-2 rounded-lg border-none bg-[var(--mythril-raised)] px-3 py-1.5 text-sm shadow-[var(--mythril-shadow-soft)] hover:bg-accent"
    >
...
// Render @mythril mentions as chips, [label](target) links as buttons, and
...
function TextWithMentions({ text, onOpenLink }: { text: string; onOpenLink?: (link: TodoLink) => void }) {
  const parts = text.split(/(@mythril|\[[^\]]+\]\([^)]+\)|\*\*[^*\n]+\*\*|`[^`\n]+`)/i)
  return (
    <>
      {parts.map((part, i) => {
        if (/^@mythril$/i.test(part)) {
          return (
...
// ---------------------------------------------------------------------------
// Receipt rows — the durable record of what @mythril did
// ---------------------------------------------------------------------------
...
function SubComposer({ onSubmit, onCancel, onHandoff }: {
  ...
  useEffect(() => {
    if (onHandoff && mentionsMythril(text)) {
      onHandoff(text)
      setText('')
    }
  }, [text, onHandoff])
  return (
    <div className="relative mt-1 flex items-center gap-2 rounded-lg border border-dashed border-border px-2.5 py-1.5">
      ...
      <input
        ...
        placeholder="Add a step… mention @mythril to hand it off"
...
function CommentComposer({ onSend, onCancel }: {
  ...
  return (
    <div className="relative mt-1.5 flex items-center gap-2 rounded-lg border border-transparent bg-[var(--mythril-wash)] px-2.5 py-1.5 focus-within:border-border">
      {mention.show && <MentionPopup onPick={mention.complete} />}
      ...
      <input
        ...
        placeholder="Tell @mythril something about this…"
        className="min-w-0 flex-1 bg-transparent text-[13px] outline-none placeholder:text-muted-foreground"
      />
    </div>
  )
...
// item's session: yours on the right, @mythril's on the left with its
...
function Bubble({ b, onOpenNote, onRetry }: {
  ...
  return (
    <div className="flex gap-2">
      ...
      >
        <span className="mr-1.5 text-[12px] font-bold">{isUser ? 'you' : 'mythril'}</span>
  ...
  return (
    <div data-todo-key={item.key} className={`group/todo relative flex items-start gap-2.5 rounded-md px-2 py-[5px] transition-colors duration-100 hover:bg-foreground/[0.045] ${dimmed ? 'opacity-35' : ''} ${
      ...
      {!item.checked && item.receipts.some((r) => r.kind === 'question') && (
        ...
        <span title="Mythril needs an answer from you" className="absolute -left-3 top-[9px] h-4 w-[2.5px] rounded bg-amber-500/80" />
      )}
      ...
      <div className="min-w-0 flex-1">
        {editing ? (
          ...
          >
            ...
            {item.proposed && (
              <span className="ml-2 rounded-full bg-muted px-1.5 py-0.5 align-middle text-[10px] font-medium text-muted-foreground">via mythril</span>
            )}
            ...
            {/* One-click delegation — prepends the mention, which routes
                through the same "typed @mythril" go path. */}
            {!item.delegated && !item.checked && !isRunning && (
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); onCommitText(`@mythril ${item.text}`) }}
      ...
      <div className="pointer-events-none absolute right-1 top-1 z-10 flex items-center gap-0.5 rounded-md border-none bg-[var(--mythril-raised)] p-0.5 opacity-0 shadow-[var(--mythril-shadow-soft)] transition-opacity duration-100 focus-within:pointer-events-auto focus-within:opacity-100 group-hover/todo:pointer-events-auto group-hover/todo:opacity-100">
        {!showConversation && (
          <IconTip label="Reply — tell @mythril something about this">
            <button
              type="button"
...
// ---------------------------------------------------------------------------
// Composer — with the @mythril autocomplete popup
// ---------------------------------------------------------------------------
...
// or by meaning: an @mythril mention here creates a delegated item, and
...
function Composer({ onSubmit }: { onSubmit: (text: string, kind: 'task' | 'chat') => void }) {
  const [text, setText] = useState('')
  const mention = useMention(text, setText)
  const isTask = mentionsMythril(text)
    ...
    <div className="relative">
      ...
      <div className="flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2.5">
        <input
          ...
          placeholder="Ask anything… mention @mythril to hand off a task"
  ...
  return (
    <div className="group/add relative mt-0.5 flex items-center gap-2.5 rounded-md px-2 py-[5px] transition-colors duration-100 hover:bg-foreground/[0.03] focus-within:bg-foreground/[0.03]">
      ...
      <input
        ...
        placeholder="Add a to-do… @mythril hands it off"
...
export function TodoView({ onOpenNote, onOpenInChat, onFocusComposer, composer, onComposeTodo, composeTarget, getRunModel, onOpenCodeSession, attendedSessionId }: TodoViewProps) {
  ...
  const runPlannerNow = useCallback(async () => {
    ...
    try {
      ...
      void window.ipc.invoke('todo:get', null).then((r) => {
        if (r.suggestions.length <= before) toast('Nothing new worth suggesting', { description: 'Mythril looked and came back empty-handed — that\'s a feature.' })
      }).catch(() => {})
    }
  ...
  const addItem = useCallback(async (text: string) => {
    ...
    await window.ipc.invoke('todo:addItem', { text, run: mentionsMythril(text) })
    await refetch()
  }, [refetch])
  ...
  const dismissKey = useCallback(async (key: string) => {
    ...
    if (res.success && res.wasProposed && itemText) {
      ...
      toast('Suggestion dismissed', {
        description: 'Mythril won\'t re-suggest this one.',
        action: {
          label: "Don't suggest things like this",
          onClick: () => {
  ...
  const addSub = useCallback(async (parentKey: string, text: string) => {
    ...
    if (dirtyRef.current) await saveNowRef.current()
    await window.ipc.invoke('todo:addSubItem', { parentKey, text, run: mentionsMythril(text) })
    await refetch()
  }, [refetch])
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background">
        ...
        <div className="mx-auto flex min-h-full max-w-[720px] flex-col gap-5">
            ...
            <div className="flex shrink-0 items-center gap-2 pt-1.5">
              ...
              {planner?.slug && (
                <div ref={plannerMenuRef} className="relative flex items-center">
                  <IconTip label="Ask Mythril for suggestions now">
                    <button
                      type="button"
                  ...
                  {plannerMenuOpen && (
                    <div className="absolute right-0 top-full z-20 mt-1 w-64 rounded-2xl border-none bg-popover p-3 shadow-[var(--mythril-shadow)]">
          ...
          <div className={`transition-opacity duration-200 ${spotSession ? 'opacity-60' : ''}`}>
                ...
                return (
                  <ItemRow
                    ...
                    onCommitText={(text) => {
                      ...
                      const updated: TodoItem = { ...item, text, key: normKey(text), delegated: mentionsMythril(text) }
                      next[index] = { kind: 'item', item: updated }
                      mutate(next)
                      // Typing @mythril into a line is the go signal.
                        ...
                        {item.children.map((child, ci) => (
                          <ItemRow
                            ...
                            onCommitText={(text) => {
                              ...
                              updateChild(index, ci, (c) => ({ ...c, text, key: newKey, delegated: mentionsMythril(text) }))
                              if (!wasDelegated && mentionsMythril(text) && !child.checked) {
            ...
            {blocks !== null && !itemBlocks.some(({ block }) => block.kind === 'item') && (
              <div className="px-2 py-2 text-[13px] text-muted-foreground/70">
                <TextWithMentions text="Nothing on the list — add your first to-do below, or mention @mythril to hand one off." />
              </div>
            )}
            {blocks !== null && (
              // Plain to-dos are typed in place — no chat chrome. Typing
              // @mythril hands the text off to the composer below, where
          ...
          {showCallout && (
            <div className="flex items-center gap-2 rounded-lg bg-foreground/[0.035] px-4 py-2.5 text-sm">
              ...
              <span className="flex-1">@mythril finished an item — the → line under it links to what it did. Hit ＋ on the row to refine the work with a comment; 💬 opens the whole conversation in the sidebar.</span>
              <button
                type="button"
              ...
              <div>
                {suggestions.map((text) => (
                  <div key={text} className="group/sugg flex items-center gap-2.5 rounded-md px-2 py-1.5 transition-colors duration-100 hover:bg-amber-500/[0.07]">
                    ...
                    <IconTip label="Decline — Mythril learns from this">
                      <button
                        type="button"
          ...
          <div className="mt-auto flex flex-col gap-5">
            ...
            <div className="text-[11px] text-muted-foreground/60">
              Saved to <code className="rounded bg-muted px-1">~/.mythril/todo.md</code> — done items archive to <code className="rounded bg-muted px-1">todo/archive/</code>.
            </div>
          </div>
        </div>
      </div>
      ...
      {/* The assistant composer — hidden until something summons it:
          typing @mythril in the list hands off here, and reply/comment
