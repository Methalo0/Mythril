...
// commands, @mythril detection) expects. These tests pin that round trip.
...
describe('markdown round trip', () => {
    const cases: [string, string][] = [
        ...
        ['mythril token', '[@mythril](#mythril) summarise this'],
...
describe('formatting commands produce wire markdown', () => {
    ...
    it('a mention token parses to ONE atom node and serializes back to the same token', () => {
        const body = 'hey [@Ada Lovelace](#member:01HADA) and [@here](#here), [@mythril](#mythril) go'
        ...
        expect(mentions).toEqual([
            ...
            { kind: 'mythril', id: null, label: 'mythril' },
        ])
