...
// @here (notify everyone online), @mythril, the org's spaces (#Name
// references) and — once a query exists — files from every shared space
// (picked files land as plain markdown links),
// and the moment the draft addresses @mythril, a strip of agent options
...
/** Per-turn agent options, sent with the invocation when the draft addresses @mythril. */
...
/** Built into the composer itself: /ask rewrites to an @mythril message and sends. */
const ASK_COMMAND: CommandEntry = { name: 'ask', args: '<question>', hint: 'Ask your Mythril — same as @mythril' }
...
export function Composer({ placeholder, onSend, onSchedule, onCreatePoll, busy, autoFocus, onType, seed, draftKey, commands = [] }: {
    ...
    onType?: () => void
    /** Prefill (e.g. "Ask @mythril about this"); a new nonce re-applies it. `append` adds to the draft instead of replacing it. */
    ...
    // slash commands, @mythril detection and buildBody all keep reading the
    ...
    // Agent options — only meaningful (and only shown) when @mythril is addressed.
    ...
    // replaces it (quote-reply, ask-mythril). Caret lands at the end.
    useEffect(() => {
    ...
    const insertMythrilChip = () => {
        ...
        editor
            ...
            .insertContent([
                ...
                { type: 'mention', attrs: { kind: 'mythril', id: null, label: 'mythril' } },
                { type: 'text', text: ' ' },
            ])
            .run()
    }
    ...
    // --- send ----------------------------------------------------------------
    const mentioned = containsMythrilAddress(draft)

    /** Per-turn agent options — attached whenever the outgoing text addresses @mythril. */
    const agentOptionsFor = (text: string): AgentOptions | undefined =>
        containsMythrilAddress(text)
            ? {
    ...
    const send = async (textOverride?: string) => {
        ...
        if (textOverride === undefined) {
            ...
            if (m && found) {
                ...
                await send(`[@mythril](#mythril) ${args}`)
                return
            }
        }
        ...
        const body = buildBody(raw)
        if (!body) return
        // From the text actually going out — an /ask rewrite mentions @mythril
    ...
    const startRecording = () => {
        ...
        void voice.start().then((result) => {
            if (result === 'mic-denied') {
                ...
                window.dispatchEvent(new CustomEvent('mythril:permission-needed', { detail: { kind: 'microphone' } }))
            }
        })
    }
        ...
        <div className="spaces-composer-dock shrink-0">
            ...
            >
                ...
                <div className={cn(recording && 'hidden')}>
                    {showCommands && (
                        <div className="absolute bottom-full left-0 right-0 z-20 mb-1.5 overflow-hidden rounded-2xl border-none bg-popover p-1.5 shadow-[var(--mythril-shadow)]">
                    ...
                    {!showCommands && activeCommand && !showMentions && (
                        <div className="absolute bottom-full left-0 right-0 z-20 mb-1.5 rounded-2xl border-none bg-popover px-3 py-2 text-xs text-muted-foreground shadow-[var(--mythril-shadow)]">
                    ...
                    <div className="spaces-compose-actions flex flex-wrap items-center gap-1.5 px-2 pb-2">
                        ...
                        <button
                            type="button"
                            onClick={insertMythrilChip}
                            title="Address your Mythril — it acts only when asked"
                            className={cn(
                        ...
                        {mentioned && (
                            <>
                                ...
                                <span className="text-[11px] text-muted-foreground">runs as your Mythril</span>
