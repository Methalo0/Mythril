...
const org = { id: 'org-1', name: 'MetHalo11885260', memberId: 'ramnique', spaces: [], directs: [] } as unknown as OrgWithSpaces
const now = new Date().toISOString()
const msg = (id: string, body: string, author = 'harsh', threadRoot?: string): spaces.Message =>
    ({ id, spaceId: 'road', author: { memberId: author, actingMode: 'direct' }, body, postedAt: now, offset: 5, replyCount: 0, reactions: [], mentions: [], mentionsHere: false, mentionsMythril: false, ...(threadRoot ? { threadRoot } : {}) }) as unknown as spaces.Message
...
const page: spaces.SpacesActivityPage = {
    items: [
        ...
        { id: 'm:2', kind: 'reply', spaceId: 'road', spaceKind: 'shared', spaceName: 'Roadboard', threadRootId: 'root', message: msg('2', 'done', 'arjun', 'root'), actors: [{ memberId: 'arjun', actingMode: 'agent', agentName: 'Mythril' }], at: now, unread: false },
...
describe('labels', () => {
    ...
    it('name actors, agents as their person’s Mythril, and fold crowds', () => {
        expect(actorLabel([{ memberId: 'harsh', actingMode: 'direct' }], names)).toBe('Harsh')
        expect(actorLabel([{ memberId: 'arjun', actingMode: 'agent' }], names)).toBe("Arjun's Mythril")
...
describe('ActivityView', () => {
    it('renders the page’s rows and opens the message a row is about', async () => {
        ...
        expect(screen.getByText("Arjun's Mythril")).toBeTruthy()
