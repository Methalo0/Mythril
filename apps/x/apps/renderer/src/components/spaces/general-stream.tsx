...
import { maybeInvokeMythril } from '@/lib/spaces-mythril'
...
export function GeneralStream({
    ...
    const post = async (body: string, agent?: AgentOptions) => {
        ...
        void window.ipc
            ...
            .then((result) => {
                ...
                analytics.spacesMessagePosted({ kind: 'general', mentionsMythril: containsMythrilAddress(body) })
                // @mythril on a fresh stream message: the agent works the thread
                // under it — its receipt lands as the first reply.
                maybeInvokeMythril(org, space, { rootMessageId: result.message.id, label: threadLabelOf(body) }, result.message.id, body, agent)
            })
            .catch(() => {
    ...
    // "Open agent chat" on one of your Mythril's posts: the run that wrote it.
    ...
    // The working strip's stop square: cancel your Mythril's run right here.
    // The chip clears when the cancelled turn releases its presence lease.
    const stopAgent = async (rootMessageId: string) => {
        try {
            const { stopped } = await window.ipc.invoke('spaces:stopMythril', { orgId: org.id, spaceId: space.id, threadRootId: rootMessageId })
    ...
    const askMythril = (message: spaces.Message) => {
    ...
    // Image embeds drop (a quote is text); names, not wire ids, like askMythril.
    ...
    const createPoll = async (input: spaces.SpacesNewPollInput) => {
        try {
            ...
            analytics.spacesMessagePosted({ kind: 'general', mentionsMythril: false })
        } catch (err) {
        ...
        <section className="flex-1 min-w-0 min-h-0 flex flex-col">
                ...
                {stream.ready && !snapping && rows.length === 0 && (
                    <div className="px-2 py-6 text-sm text-muted-foreground">
                        {space.kind === 'direct' && (space.participants ?? []).length === 1
                            ? 'Your notes to self — drafts, links, files for later. Only you can see this, and @mythril works here too.'
                            : space.kind === 'direct'
                                ? 'Private to the two of you — say hello, or @mythril to ask your agent.'
                                : 'Nothing here yet — say hello, or @mythril to ask your agent.'}
                    </div>
                )}
                {rows}
            ...
            {detached ? (
                ...
                <button
                    ...
                    disabled={snapping}
                    className="absolute bottom-3 left-1/2 z-20 inline-flex -translate-x-1/2 animate-in fade-in slide-in-from-bottom-2 items-center gap-1.5 rounded-full border-none bg-[var(--mythril-raised)] px-3 py-1 text-xs font-medium shadow-[var(--mythril-shadow-soft)] hover:bg-accent disabled:opacity-60"
                >
                ...
                return (
                    <button
                        ...
                        className="absolute bottom-3 left-1/2 z-20 inline-flex -translate-x-1/2 animate-in fade-in slide-in-from-bottom-2 items-center gap-1.5 rounded-full border-none bg-[var(--mythril-raised)] px-3 py-1 text-xs font-medium shadow-[var(--mythril-shadow-soft)] hover:bg-accent"
                    >
            ...
            <Composer
                placeholder={`Message ${space.name} — @mythril to ask your agent`}
                busy={false}
                draftKey={memoryKey}
                onSend={post}
