...
// control, the dev add-org dialog, and the @mythril trigger.
...
export function AddOrgDialog({ open, onOpenChange, onAdded, initialAction, initialInviteUrl }: {
    ...
    // One dialog. With no Mythril session the first door is "Sign in with
    // Mythril" — one browser trip, then every managed org the person belongs
    ...
    const signInMythril = async () => {
        ...
        setWaiting('signin')
        try {
            const { orgs } = await window.ipc.invoke('spaces:signInMythril', null)
            refreshSpacesAccountState()
            void refreshSpacesOrgs()
            toast(orgs.length > 0 ? `Signed in — ${orgs.length} ${orgs.length === 1 ? 'server' : 'servers'} ready` : 'Signed in to Mythril', 'success')
            if (orgs.length > 0) {
                onOpenChange(false)
                onAdded(orgs[0].id)
            }
                ...
                {mode === 'main' ? (
                    <div className="space-y-3">
                        {account && !account.hasSession && (
                            <>
                                <div>
                                    <div className="text-sm font-medium">Sign in with Mythril</div>
                                    <p className="text-xs text-muted-foreground">Your servers appear once you’re signed in.</p>
                                    <Button onClick={() => void signInMythril()} disabled={busy} className="mt-1.5 w-full">
                                        {waiting === 'signin' && <Loader2 className="size-3.5 mr-1 animate-spin" />} Sign in with Mythril
                                    </Button>
                                </div>
                    ...
                    <div className="space-y-3">
                        <div>
                            ...
                            />
                            <p className="mt-1 text-xs text-muted-foreground">A URL, a host, or a Mythril server’s slug. You need to already be a member — otherwise ask for an invite link.</p>
                        </div>
                        {waiting === 'address' && (
                            <div className="text-xs text-muted-foreground flex items-center gap-1.5">
                                <Loader2 className="size-3 animate-spin" /> Waiting for the browser sign-in…
