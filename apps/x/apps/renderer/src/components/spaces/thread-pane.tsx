...
export function ThreadPane({
    ...
    /** What @mythril and sessions call this conversation. */
    ...
    const post = async (body: string, agent?: AgentOptions) => {
        ...
        void window.ipc
            ...
            .then((result) => {
                ...
                analytics.spacesMessagePosted({ kind: 'topic', mentionsMythril: containsMythrilAddress(body) })
                maybeInvokeMythril(org, space, { rootMessageId, label: threadLabel }, result.message.id, body, agent)
            })
            .catch(() => {
                setMessages((prev) => {
    ...
    const fold = async (file: { assetId: string; path: string }) => {
        ...
        onFolding?.(true)
        try {
            const body = `[@mythril](#mythril) fold this thread’s decision into \`${file.path}\` (assetId ${file.assetId}) — keep the file’s structure and put it under the right section. End your change reason with “· thread:${rootMessageId}”.`
            ...
            maybeInvokeMythril(org, space, { rootMessageId, label: threadLabel }, result.message.id, body)
        } catch (err) {
            toast(err instanceof Error ? err.message : 'Could not ask Mythril', 'error')
        } finally {
            setFolding(false)
            onFolding?.(false)
        }
    ...
    const createPoll = async (input: spaces.SpacesNewPollInput) => {
        try {
            ...
            analytics.spacesMessagePosted({ kind: 'topic', mentionsMythril: false })
        } catch (err) {
    ...
    // "Open agent chat" on one of your Mythril's replies: the run that wrote
                ...
                {permissionWait.length > 0 && (
                    <div className="flex flex-wrap items-center gap-2 pl-10 pt-1">
                        ...
                        >
                            ...
                            Your Mythril needs permission — {permissionWait[0]}
                ...
                {spinningAgents.length > 0 && (
                    <div className="flex flex-wrap items-center gap-2 pl-10 pt-1">
                        {spinningAgents.map((memberId) => {
                            ...
                            const label = own ? 'Your Mythril is working…' : <><MemberName id={memberId} />’s Mythril is working…</>
                            return own ? (
                ...
                commands={[
                    {
                        name: 'fold',
                        args: '<file>',
                        hint: 'Ask your Mythril to fold this thread into a file',
                        run: (args) => {
