...
// @mythril detection, buildBody) keeps operating on the same markdown string
 ...
 * text handed to @mythril) showed them unconditionally. A newline carries the
...
type MentionKind = 'member' | 'space' | 'here' | 'mythril'
...
/** The pill's text: `#Name` for a space reference, `@Name` for a person, `@here` / `@mythril` for the fixed addresses. */
 ...
 * `[@mythril](#mythril)`, `[#Name](#space:<id>)` — protocol mentions.ts) and
...
export const MentionNode = Node.create({
    ...
    parseHTML() {
        return [
            ...
            { tag: 'a[href="#mythril"]', priority: 1001, getAttrs: () => ({ kind: 'mythril', id: null, label: 'mythril' }) },
            {
