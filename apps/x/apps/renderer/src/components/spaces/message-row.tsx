...
const MENU_HIGHLIGHT =
    '[&_[role^=menuitem]:focus]:bg-[var(--mythril-wash)] [&_[role^=menuitem]:focus]:text-[var(--stream-link)] [&_[role^=menuitem]:focus_svg]:text-[var(--stream-link)]!'
const ICON_HOVER =
    'transition-colors hover:bg-[var(--mythril-wash)] hover:text-[var(--stream-link)] active:bg-[var(--stream-mention-wash)] data-[state=open]:bg-[var(--mythril-wash)] data-[state=open]:text-[var(--stream-link)]'
...
function MessageRowImpl({
    ...
    /** Names the viewer's own agent "Your Mythril" on thread rows. */
    selfMemberId?: string
    ...
    /**
     * Opens the run that posted THIS message (the viewer's own Mythril's
     * reply), landing on its turn — the "via Mythril" label and the menu's
    ...
    /** Stops the viewer's own Mythril working this thread (working strip's stop square). */
    onStopAgent?: (rootMessageId: string) => void
    onReplyInThread?: (message: spaces.Message) => void
    onAskMythril?: (message: spaces.Message) => void
    ...
    const showActions = !deleted && !unconfirmed && !!(onReplyInThread || onAskMythril || onCopyLink || onReact || canDelete)
    ...
    const row = (
        ...
        >
            ...
            <div className="min-w-0 flex-1">
                {!continuation && (
                    <div className="flex flex-wrap items-baseline gap-x-2 text-xs">
                        ...
                        {viaAgent && (
                            canOpenResponseChat ? (
                                // Your own Mythril's post: the label is the subtle way in
                ...
                {thread && thread.replyCount > 0 && onOpenThread && (
                    ...
                    <button
                        ...
                        className={cn(
                            'spaces-reply-link group/thread mt-1 flex w-fit max-w-full flex-wrap items-center gap-x-2 gap-y-1 rounded border border-transparent px-1 py-1 text-left text-xs transition-colors hover:border-border hover:bg-[var(--mythril-raised)]',
                    ...
                    >
                        ...
                        {thread.unreadCount > 0 && <span className="font-semibold text-[var(--mythril-attention)]">New replies</span>}
                            ...
                            <span className="inline-flex items-center gap-1 text-muted-foreground">
                                ...
                                {thread.workingAgents.length === 1
                                    ? thread.workingAgents[0] === selfMemberId
                                        ? 'Your Mythril is working…'
                                        : `${memberNames.get(thread.workingAgents[0]!) ?? thread.workingAgents[0]}’s Mythril is working…`
                ...
                {thread && thread.replyCount === 0 && thread.workingAgents.length > 0 && onOpenThread && (
                    <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                        ...
                        >
                            <Loader2 className="size-3 animate-spin" /> Mythril is working on a reply…
                        </button>
                        {onOpenAgentChat && (
                            <button
                                type="button"
            ...
            {showActions && (
                <div className={cn('spaces-message-actions absolute right-4 flex items-center rounded-md border border-border bg-[var(--mythril-raised)] p-1 shadow-sm', (pickerOpen || menuOpen) && 'spaces-message-actions--open')}>
                    {onReact && (
                    ...
                    {onAskMythril && (
                        <button
                            type="button"
                            title="Ask @mythril about this"
                            onClick={() => onAskMythril(message)}
                            className={cn('inline-flex size-8 items-center justify-center rounded text-muted-foreground', ICON_HOVER)}
                        >
    ...
    const hasTopItems = !!(onReact || onReplyInThread || onAskMythril || onCopyLink || messageText)
    return (
        <ContextMenu onOpenChange={(open) => { if (open) setSelectionText(window.getSelection()?.toString().trim() ?? '') }}>
            ...
            <ContextMenuContent className={cn('w-56', MENU_HIGHLIGHT)}>
                ...
                {onAskMythril && (
                    <ContextMenuItem onSelect={() => onAskMythril(message)}>
                        <Bot className="size-3.5 mr-2" /> Ask @mythril about this
                    </ContextMenuItem>
                )}
                {canEdit && (
...
export function NewDivider({ fading = false }: { fading?: boolean }) {
    return (
        ...
        <div data-new-divider className={cn('flex items-center gap-2.5 px-2 py-1 transition-opacity duration-700', fading && 'opacity-0')}>
            <span className="h-px flex-1 bg-[var(--mythril-attention)]" />
            <span className="text-[11px] font-medium text-[var(--mythril-attention)]">New</span>
        </div>
    )
}
