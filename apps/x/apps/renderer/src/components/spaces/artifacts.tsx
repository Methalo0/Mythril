        ...
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <button
                    ...
                    title="Ask your Mythril to fold this topic’s decision into a file"
                >
            ...
            <div className="flex flex-1 min-h-0 flex-col gap-1.5 overflow-y-auto px-2.5 pt-2.5 pb-2">
                {groups.length === 0 && !working && (
                    <div className="rounded-lg border border-dashed border-border px-3 py-3 text-xs text-muted-foreground">
                        Nothing changed from this topic yet. Ask @mythril to fold a decision into a file, or pick one above.
                    </div>
                )}
                {groups.map((g) => (
                ...
                {working && (
                    <div className="flex items-center gap-2 px-1 py-1 text-xs text-muted-foreground"><Loader2 className="size-3 animate-spin" /> A Mythril is working in this topic…</div>
                )}
            </div>
