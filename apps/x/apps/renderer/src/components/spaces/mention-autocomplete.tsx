...
export function useMentionAutocomplete(editor: Editor | null) {
    ...
    const candidates = useMemo<MentionCandidate[]>(() => {
        ...
        if ('mythril'.startsWith(q)) people.push({ id: 'mythril', label: 'mythril', hint: 'your agent — acts only when asked', isAgent: true })
    ...
    // A person, a space, @mythril, or @here becomes ONE mention node
...
export function MentionMenu({ anchor, candidates, index, onPick }: {
    ...
    return createPortal(
        <div
            ...
            className="z-50 max-h-80 w-72 overflow-y-auto rounded-2xl border-none bg-popover p-1.5 shadow-[var(--mythril-shadow)]"
        >
            {candidates.map((c, i) => (
                <button
                    key={c.id}
