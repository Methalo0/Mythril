...
describe("AboutDialog", () => {
  it("shows product identity, versions, and live update status", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Mythril" })).toBeInTheDocument()
    expect(screen.getByRole("img", { name: "Mythril logo" })).toHaveAttribute("src", "./logo-only.png")
  ...
  it("checks for updates and copies diagnostics", async () => {
    ...
    expect(writeText.mock.calls[0][0]).toContain("Mythril 1.2.3")
  ...
  it("offers the installed update restart action", async () => {
    ...
    expect(await screen.findByText("Mythril 1.3.0 is ready to install.")).toBeInTheDocument()
