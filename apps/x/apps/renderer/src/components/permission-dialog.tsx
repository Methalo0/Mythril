...
> = {
  microphone: {
    icon: Mic,
    title: 'Mythril needs microphone access',
    body:
      `Voice input is off because ${OS} is blocking the microphone for Mythril. ` +
      `Enable it under ${MIC_PATH}, then try again.`,
    section: 'microphone',
  },
  camera: {
    icon: Video,
    title: 'Mythril needs camera access',
    body:
      `Video calls are off because ${OS} is blocking the camera for Mythril. ` +
  ...
  'screen-recording': {
    icon: MonitorUp,
    title: 'Mythril can’t see your screen',
    body: isMac
      ? 'macOS is blocking Screen Recording, so the assistant would only see black frames. ' +
        'Enable Mythril under System Settings → Privacy & Security → Screen Recording, then ' +
        'relaunch Mythril. If Mythril is already enabled there, toggle it off and on — an ' +
      ...
      : 'Screen capture didn’t start, so the assistant can’t see your screen. If you ' +
        ...
        'Mythril — the call carries on fine without sharing.',
  ...
  'input-monitoring': {
    ...
    body:
      ...
      'another app. For Mythril to see that key outside its own window, macOS requires ' +
