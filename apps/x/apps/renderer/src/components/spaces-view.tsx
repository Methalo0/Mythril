...
export function SpacesView({ selection, onSelect, onSwitchSpace, railSelection, onRailSelect, onOpenSession, onOpenMessage, onOpenActivity, active = true }: {
    ...
    // No Mythril session → the empty state offers the sign-in first (one
    // session, two uses): one browser trip lists every managed org.
    const account = useSpacesAccountState()
    const [signingIn, setSigningIn] = useState(false)
    const signInMythril = async () => {
        setSigningIn(true)
        try {
            const { orgs: signedIn } = await window.ipc.invoke('spaces:signInMythril', null)
    ...
    if (selectedOrg) {
        return <div className="spaces-surface flex min-h-0 flex-1 flex-col">
            ...
            <aside className="w-64 shrink-0 overflow-y-auto border-r border-border bg-[var(--mythril-panel-soft)] p-2">
                    ...
                    <>
                        ...
                        {account && !account.hasSession ? (
                            <div className="mt-4 flex flex-col items-center gap-2">
                                <Button size="sm" onClick={() => void signInMythril()} disabled={signingIn}>
                                    {signingIn ? <Loader2 className="size-4 mr-1 animate-spin" /> : null} Sign in with Mythril
                                </Button>
                    ...
                    <PopoverTrigger asChild>
                        ...
                        >
                            <span className="relative">
                                ...
                                {here.length > 0 && <span className="absolute -right-1 -top-1 size-2 rounded-full bg-[var(--mythril-success)] ring-2 ring-background" />}
                            </span>
                                    ...
                                    <MemberProfilePopover key={m.id} id={m.id}>
                                        <button type="button" className="flex w-full cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-left hover:bg-accent/60">
                                            <span className="relative shrink-0">
                                                ...
                                                {isHere && <span className="absolute -bottom-0.5 -right-0.5 size-2 rounded-full bg-[var(--mythril-success)] ring-2 ring-popover" />}
                                            </span>
                                            ...
                                            {isHere && <span className="shrink-0 text-[10.5px] text-[var(--mythril-success)]">here</span>}
                                        </button>
...
function CopyLine({ text, title, className }: { text: string; title: string; className?: string }) {
    ...
    return (
        ...
        >
            ...
            {copied
                ? <Check className="size-3 shrink-0 text-[var(--mythril-success)]" />
