...
const WEBSITE_URL = "https://www.github.com/MetHalo11885260/"
const RELEASES_URL = "https://github.com/Methal0/Mythril/releases"
const REPO_URL = "https://github.com/Methal0/Mythril"
const SUPPORT_URL = "mailto:contact@github.com/MetHalo11885260"
...
function AboutUpdate({ status }: { status: UpdaterStatus | null }) {
  ...
  if (status.state === "ready") {
    ...
    return (
      <div className="flex min-h-14 items-center gap-3 rounded-xl border border-border/60 bg-background/80 px-3 py-2.5">
        <span className="size-2.5 shrink-0 rounded-full bg-[var(--mythril-attention)]" aria-hidden />
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium">Update ready</p>
          <p className="truncate text-xs text-muted-foreground">
            {version ? `Mythril ${version} is ready to install.` : "A new version is ready to install."}
          </p>
        </div>
    ...
    return (
      <div className="flex min-h-14 items-center gap-3 rounded-xl border border-border/60 bg-background/80 px-3 py-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-muted-foreground">Get the latest Mythril release from GitHub.</p>
        </div>
  ...
  return (
    <div className="flex min-h-14 items-center gap-3 rounded-xl border border-border/60 bg-background/80 px-3 py-2.5">
      <CheckCircle2 className="size-4 shrink-0 text-[var(--mythril-success)]" />
      <div className="min-w-0 flex-1">
...
export function AboutDialog({ open, onOpenChange }: AboutDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="gap-0 overflow-hidden p-0 sm:max-w-[540px]" aria-describedby="about-mythril-description">
        <header className="grid justify-items-center px-8 pb-8 pt-10 text-center">
          <img src="./logo-only.png" alt="Mythril logo" width={82} height={82} className="size-[82px] object-contain" />
          ...
          <DialogDescription id="about-mythril-description" className="mt-3 max-w-[360px] text-sm leading-relaxed">
        ...
        <div className="px-7 pb-6">
          ...
          <nav className="mt-4 flex flex-wrap justify-center gap-x-5 gap-y-2 text-xs font-medium" aria-label="About Mythril links">
          ...
          <details className="group mt-4 border-y border-border/60">
            ...
            <dl className="grid grid-cols-[1fr_auto] gap-x-4 gap-y-2 pb-3 text-xs">
              <dt className="text-muted-foreground">Mythril</dt><dd>Version {appVersion}</dd>
          ...
          <p className="mt-4 text-center text-[0.7rem] text-muted-foreground">Made by MetHalo11885260 · Apache 2.0</p>
        </div>
      </DialogContent>
    </Dialog>
  )
