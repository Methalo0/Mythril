...
const GATEWAY_IMAGE_PROVIDER: ImageProvider = {
  id: 'mythril', flavor: 'mythril', status: 'ok', models: ['google/gemini-2.5-flash-image'],
}
...
function serve(opts: {
  ...
  handlers['models:list'] = async () => ({
    providers: [
      { id: 'mythril', flavor: 'mythril', status: 'ok', models: [{ id: 'google/gemini-3.5-flash' }] },
    ],
...
describe('ModelSelectionSection', () => {
  it('shows the effective assistant model and "Same as Assistant" for un-overridden tasks', async () => {
    serve({ assistant: { provider: 'mythril', model: 'google/gemini-3.5-flash' } })
    ...
    expect(screen.queryByText('Mythril default')).toBeNull()
    ...
    expect(screen.getAllByText('Currently uses Mythril · google/gemini-3.5-flash').length).toBeGreaterThan(0)
  })

  it('an overridden task shows "Use Assistant model" and clicking it clears the override', async () => {
    serve({
      assistant: { provider: 'mythril', model: 'google/gemini-3.5-flash' },
      taskModels: { knowledgeGraph: { provider: 'mythril', model: 'google/gemini-3.1-flash-lite' } },
    })
    render(<ModelSelectionSection dialogOpen />)
  ...
  it('picking an image model from the image catalog persists the ref', async () => {
    ...
    await waitFor(() => expect(updateCalls).toEqual([
      { imageModel: { provider: 'mythril', model: 'google/gemini-2.5-flash-image' } },
    ]))
    await waitFor(() => expect(screen.getByTitle('Image model')).toHaveTextContent('google/gemini-2.5-flash-image'))
    expect(screen.getByText('Currently uses Mythril · google/gemini-2.5-flash-image')).toBeInTheDocument()
  })
