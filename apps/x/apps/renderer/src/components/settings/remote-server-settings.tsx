...
// Settings → Connect to server: point this desktop at a mythril-server on
// another machine (address + access code from that machine's
// ~/.mythril/server-key), or go back to running everything locally. On a
...
export function RemoteServerSettings({ dialogOpen }: { dialogOpen: boolean }) {
    ...
    return (
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          This connection is set by environment variables (MYTHRIL_REMOTE_SERVER), so it
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Run Mythril against a server on another machine. Enter its address and the
        access code from <code className="font-mono">~/.mythril/server-key</code> on that
