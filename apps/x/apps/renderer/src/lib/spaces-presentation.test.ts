...
function cs(over: Partial<spaces.ChangeSet> & { id: string; committedAt: string }): spaces.ChangeSet {
    return {
        ...
        baseVersion: 1,
        resultVersion: 2,
        attribution: { memberId: 'ramnique', actingMode: 'agent', agentName: 'Mythril' },
        offset: 1,
        ...over,
    }
}
...
describe('initials / monograms', () => {
    ...
    it('derives an org monogram from the address, falling back to the name', () => {
        expect(orgMonogram({ name: 'MetHalo11885260', address: 'mythril.team' })).toBe('RT')
        expect(orgMonogram({ name: 'MetHalo11885260 (dev)', address: 'localhost:4272' })).toBe('RL')
    })
})
...
describe('file links — relative markdown resolves against the tree', () => {
    ...
    it('rewriteFileLinks: relative links resolve through the listing to app://space-file by id, else keep their path; images, absolute URLs, code stay', () => {
        ...
        const untouched = [
            ...
            '[canon](https://mythril.team/s/01HXAMPZESPACE00000000000A/a/A-sso)',
            '[m](mailto:a@b.c)',
            '`[c](a.md)`',
            '```\n[c](a.md)\n```',
        ]
    ...
    it('mints and parses the canonical asset URL (…/s/<spaceId>/a/<assetId>) for any space on any org', () => {
        const refs = { orgId: 'o1', orgAddress: 'mythril.team', spaceId: '01HXAMPZESPACE00000000000A' }
        const url = assetWireUrl(refs, '01HXAMPLEASSET0000000000A1')
        expect(url).toBe('https://mythril.team/s/01HXAMPZESPACE00000000000A/a/01HXAMPLEASSET0000000000A1')
        expect(parseAssetWireUrl(url)).toEqual({ orgAddress: 'mythril.team', spaceId: '01HXAMPZESPACE00000000000A', assetId: '01HXAMPLEASSET0000000000A1' })
        ...
        expect(parseAssetWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A/f/decisions/sso.md')).toBeNull()
        expect(parseAssetWireUrl(`https://mythril.team/s/01HXAMPZESPACE00000000000A/b/${'a'.repeat(64)}`)).toBeNull()
        expect(parseAssetWireUrl('https://mythril.team/s/not-a-space/a/A1')).toBeNull()
        expect(parseAssetWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A/a/')).toBeNull()
    })
})
...
describe('mentions — tokens carry ids, the roster supplies names', () => {
    ...
    it('rewriteMentionLinks turns tokens into the app links the anchor renders as chips', () => {
        expect(rewriteMentionLinks(`ping ${tok('01HXAMPLEULIDHARSH000000', 'Harsh')} [@here](#here) [@mythril](#mythril)`))
            .toBe('ping [@Harsh](app://space-member/01HXAMPLEULIDHARSH000000) [@here](app://space-mention/here) [@mythril](app://space-mention/mythril)')
    ...
    it('parseSpaceWireUrl takes the canonical space link and nothing under it', () => {
        expect(parseSpaceWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A')).toEqual({ orgAddress: 'mythril.team', spaceId: '01HXAMPZESPACE00000000000A' })
        expect(parseSpaceWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A/')?.spaceId).toBe('01HXAMPZESPACE00000000000A')
        expect(parseSpaceWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A/a/01HXAMPLEASSET0000000000A1')).toBeNull()
        expect(parseSpaceWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A/m/01HXAMPLEMSG00000000000001')).toBeNull()
        expect(parseSpaceWireUrl('https://mythril.team/s/not-a-space')).toBeNull()
        expect(parseSpaceWireUrl('http://mythril.team/s/01HXAMPZESPACE00000000000A')).toBeNull()
    })
})
...
describe('blob links', () => {
    ...
    const refs = { orgId: 'org-1', orgAddress: 'mythril.spaces.example.com', spaceId: SPACE }

    it('wire and app URL forms round-trip through the rewrite', () => {
        const wire = blobWireUrl(refs, HASH, 'design doc.pdf')
        expect(wire).toBe(`https://mythril.spaces.example.com/s/${SPACE}/b/${HASH}?name=design%20doc.pdf`)
    ...
    it('rewrites only this org, leaves code regions and foreign hosts alone', () => {
        ...
        const code = `\`https://mythril.spaces.example.com/s/${SPACE}/b/${HASH}\``
...
describe('unread changes', () => {
    it('marks a change unread when it landed after the mark (an offset) and was not my own direct edit', () => {
        ...
        const myAgent = cs({ id: 'agent', committedAt: '2026-08-19T17:30:00Z', offset: 15, attribution: { memberId: 'me', actingMode: 'agent', agentName: 'Mythril' } })
...
describe('parseMemberWireUrl / parseMessageWireUrl', () => {
    it('read the contract\'s person and message links, and only those', () => {
        expect(parseMemberWireUrl('https://mythril.team/u/google%7C123')).toEqual({ orgAddress: 'mythril.team', memberId: 'google|123' })
        expect(parseMessageWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A/m/01HXAMPZEMSG000000000000A1')).toEqual({
            orgAddress: 'mythril.team',
            spaceId: '01HXAMPZESPACE00000000000A',
            messageId: '01HXAMPZEMSG000000000000A1',
        })
        expect(parseMemberWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A')).toBeNull()
        expect(parseMessageWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A/a/x')).toBeNull()
        expect(parseMessageWireUrl('https://mythril.team/s/01HXAMPZESPACE00000000000A/m/not-a-ulid')).toBeNull()
        expect(parseMemberWireUrl('https://mythril.team/u/')).toBeNull()
    })
})
