 ...
 * app as mythril://open?type=spaces&org=<address>&…: a space, a file in it, a
...
export function parseSpacesLink(input: string): SpacesLinkTarget | null {
  // Some OS handlers normalise the authority form to mythril://open/?… — the
  // same tolerance every other mythril:// parser has.
  const m = /^mythril:\/\/open\/?\?(.*)$/.exec(input)
  if (!m) return null
