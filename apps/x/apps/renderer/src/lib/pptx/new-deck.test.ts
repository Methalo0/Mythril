...
const V1_THEME =
  ...
  '<a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="Mythril Navy">' +
...
describe('upgradeGeneratedDeck', () => {
  ...
  it('leaves fresh v2 decks and foreign decks alone', async () => {
    ...
    for (const [p, xml] of newDeckParts({ title: 'T', palette: NAVY })) {
      foreign.file(p, p === 'ppt/theme/theme1.xml' ? V1_THEME.replace('Mythril Navy', 'Office Theme') : xml)
    }
