...
describe('org link landings → app deep links', () => {
    it('reads the org address and whichever target the landing named', () => {
        expect(parseSpacesLink('mythril://open?type=spaces&spaceId=S1&org=acme.mythril.space')).toEqual({ orgAddress: 'acme.mythril.space', spaceId: 'S1' })
        expect(parseSpacesLink('mythril://open?type=spaces&spaceId=S1&messageId=M1&org=acme.mythril.space')).toEqual({ orgAddress: 'acme.mythril.space', spaceId: 'S1', messageId: 'M1' })
        // The /join landing: an invite to join, not a place to go.
        expect(parseSpacesLink('mythril://open?type=spaces&org=acme.mythril.space&invite=t0k3n')).toEqual({ orgAddress: 'acme.mythril.space', inviteToken: 't0k3n' })
        expect(parseSpacesLink('mythril://open?type=spaces&spaceId=S1&assetId=A%2Fx&org=acme.mythril.space')).toEqual({ orgAddress: 'acme.mythril.space', spaceId: 'S1', assetId: 'A/x' })
        expect(parseSpacesLink('mythril://open?type=spaces&memberId=google%7C1&org=acme.mythril.space')).toEqual({ orgAddress: 'acme.mythril.space', memberId: 'google|1' })
    ...
    it('leaves orgId links (notifications) and every other deep link to the plain parser', () => {
        expect(parseSpacesLink('mythril://open?type=spaces&orgId=org&spaceId=S1')).toBeNull()
        expect(parseSpacesLink('mythril://open?type=file&path=knowledge/a.md')).toBeNull()
        expect(parseSpacesLink('https://acme.mythril.space/s/S1')).toBeNull()
    })
})
