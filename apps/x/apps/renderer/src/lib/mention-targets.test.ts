...
const space = (name: string, org = 'mythril'): SpaceMentionTarget => ({
  kind: 'space',
  orgId: `org-${org}`,
  orgName: org,
...
const person = (displayName: string, org = 'mythril'): MemberMentionTarget => ({
  kind: 'member',
  orgId: `org-${org}`,
  orgName: org,
...
const board = (name: string, spaceName = 'Design', org = 'mythril'): BoardMentionTarget => ({
  kind: 'board',
  orgId: `org-${org}`,
  orgName: org,
...
describe('buildMentionEntries', () => {
  it('browses each group in turn on a bare "@": mythril, files, spaces, people', () => {
    const entries = buildMentionEntries('', sources)
    expect(entries[0].target.kind).toBe('mythril')
  ...
  it('filters every group by the query, case-insensitively, and drops mythril once it stops matching', () => {
    const entries = buildMentionEntries('DE', sources)
    expect(entries.some((e) => e.target.kind === 'mythril')).toBe(false)
  ...
  it('keeps mythril first while the query is a prefix of it', () => {
  ...
  it('scopes keys by org, so the same member on two orgs stays two entries', () => {
    const entries = buildMentionEntries('harsh', {
      ...sources,
      members: [person('Harsh Kumar', 'mythril'), person('Harsh Kumar', 'acme')],
    })
