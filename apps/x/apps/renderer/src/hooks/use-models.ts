...
// catalog (models:list → core/models/catalog.ts). Every provider — Mythril
...
export interface ModelPickerGroup {
  ...
  id: string
  /** Provider type ("openai", "ollama", "mythril", …) — display naming. */
  flavor: string
  models: string[]
...
export interface ModelsSnapshot {
  ...
  defaultEffort: '' | 'low' | 'medium' | 'high' | 'xhigh' | 'ultrahigh'
...
async function buildSnapshot(refreshProvider?: string): Promise<ModelsSnapshot> {
  ...
  return {
    ...
    defaultModel,
    defaultEffort,
    isMythrilConnected: catalog.providers.some((p) => p.id === 'mythril'),
    catalogByProvider,
  }
}
...
function wireGlobalEvents(): void {
  ...
  wiredCleanups = [
    ...
    // Mythril sign-in/out swaps the provider set. Despite the name, main
