...
import { useMythrilAccount } from '@/hooks/useMythrilAccount';
import { fetchMythrilConfig } from '@/hooks/use-mythril-config';
...
export function useMeetingTranscription(onAutoStop?: () => void) {
    const { refresh: refreshMythrilAccount } = useMythrilAccount();
    ...
    const start = useCallback(async (calendarEvent?: CalendarEventMeta): Promise<string | null> => {
        ...
        const [headphoneResult, wsResult, micResult, systemResult] = await Promise.allSettled([
            ...
            (async () => {
                ...
                const [account, mythrilConfig] = await Promise.all([
                    refreshMythrilAccount(),
                    fetchMythrilConfig(),
                ]);
                let ws: WebSocket;
                if (
                    account?.signedIn &&
                    account.accessToken &&
                    mythrilConfig?.websocketApiUrl
                ) {
                    const listenUrl = buildDeepgramListenUrl(mythrilConfig.websocketApiUrl, DEEPGRAM_PARAMS);
                    console.log('[meeting] Using Mythril WebSocket');
        ...
        const filename = calendarEvent?.summary
            ...
            : `meeting-${timestamp}`;
        let notePath = `knowledge/Meetings/mythril/${dateFolder}/${filename}.md`;
        ...
        if (calendarEvent?.summary) {
            try {
                ...
                if (exists) notePath = `knowledge/Meetings/mythril/${dateFolder}/${filename}-${timestamp}.md`;
        ...
        setState('recording');
        return notePath;
    }, [state, cleanup, scheduleDebouncedWrite, refreshMythrilAccount]);
