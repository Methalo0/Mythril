...
export function actionableSlackError(kind?: string, message?: string): string {
  ...
  switch (kind) {
    case 'not_installed':
      return 'The Slack helper is unavailable in this build. Please update or reinstall Mythril.'
    case 'network':
...
export function useConnectors(active: boolean) {
  ...
  const startConnect = useCallback(async (provider: string, credentials?: { clientId: string; clientSecret: string }) => {
    ...
    try {
      ...
      if (!result.success) {
        toast.error(result.error || (provider === 'mythril' ? 'Failed to log in to Mythril' : `Failed to connect to ${provider}`))
        setProviderStates(prev => ({
          ...prev,
      ...
      toast.error(provider === 'mythril' ? 'Failed to log in to Mythril' : `Failed to connect to ${provider}`)
      setProviderStates(prev => ({
        ...prev,
        [provider]: { ...prev[provider], isConnecting: false }
      }))
  ...
  const handleConnect = useCallback(async (provider: string) => {
    if (provider === 'google') {
      // Signed-in users use the mythril (managed-credentials) flow: opens
      ...
      const isSignedIntoMythril = providerStates.mythril?.isConnected ?? false
      if (isSignedIntoMythril) {
  ...
  // mythril-vs-BYOK branching for Google so signed-in users don't get the
  ...
  const handleDisconnect = useCallback(async (provider: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success(provider === 'mythril' ? 'Logged out of Mythril' : `Disconnected from ${displayName}`)
        setProviderStates(prev => ({
          ...prev,
          [provider]: {
            isConnected: false,
        ...
        toast.error(provider === 'mythril' ? 'Failed to log out of Mythril' : `Failed to disconnect from ${provider}`)
        setProviderStates(prev => ({
          ...prev,
    ...
    const cleanup = window.ipc.on('oauth:didConnect', async (event) => {
      ...
      if (success) {
        const displayName = provider === 'fireflies-ai' ? 'Fireflies'
          ...
          : provider.charAt(0).toUpperCase() + provider.slice(1)
        if (provider === 'mythril') {
          toast.success('Logged in to Mythril')
