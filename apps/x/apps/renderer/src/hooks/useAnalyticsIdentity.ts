...
/**
 * Identifies the user in PostHog when signed into Mythril,
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // Identify if Mythril account is connected
        const mythril = config.mythril
        if (mythril?.connected && mythril?.userId) {
          identifyUser(mythril.userId)
        }

        // Set provider connection flags
        const providers = ['gmail', 'calendar', 'slack', 'mythril']
        const props: Record<string, boolean> = { signed_in: !!mythril?.connected }
  ...
  useEffect(() => {
    const cleanup = window.ipc.on('oauth:didConnect', (event) => {
      ...
      if (event.success) {
        ...
        posthog.people.set({ signed_in: true, mythril_connected: true })
        posthog.capture('user_signed_in')
        return
      }

      // Mythril sign-out — flip flags, capture, and reset distinct_id so
      // future events on this device don't get attributed to the prior user.
      posthog.people.set({ signed_in: false, mythril_connected: false })
