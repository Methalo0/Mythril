...
describe('useModels', () => {
  ...
  it('sign-out via the oauth:didConnect broadcast flips isMythrilConnected and drops the mythril group', async () => {
    serveCatalog({
      providers: [{ id: 'mythril', models: [{ id: 'claude-opus-4-8' }] }],
      defaultModel: { provider: 'mythril', model: 'claude-opus-4-8' },
    })

    const { result } = renderHook(() => useModels())
    await waitFor(() => expect(result.current.isMythrilConnected).toBe(true))
    expect(result.current.groups).toEqual([
      { id: 'mythril', flavor: 'mythril', models: ['claude-opus-4-8'], status: 'ok' },
    ])
    ...
    act(() => {
      for (const listener of ipcListeners['oauth:didConnect'] ?? []) {
        listener({ provider: 'mythril', success: false })
      }
    })

    await waitFor(() => expect(result.current.isMythrilConnected).toBe(false))
    expect(result.current.groups.some((g) => g.id === 'mythril')).toBe(false)
  })
