...
// ---------------------------------------------------------------------------
// Account state — is there a Mythril session, and is the app signed in on it?
// The Spaces doors key off this: no session → "Sign in with Mythril" first.
// ---------------------------------------------------------------------------
...
function wireFeedBus(): void {
    ...
    if (typeof window !== 'undefined') {
        ...
        // A Mythril sign-in or sign-out changes which managed orgs we hold
        ...
        window.ipc.on('oauth:didConnect', (event) => {
            if (event.provider !== 'mythril') return
            orgsRefreshedAt = 0
            refreshAccountState()
            void refreshSpacesOrgs()
        })
