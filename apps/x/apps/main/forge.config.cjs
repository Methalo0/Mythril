...
// but CI sets MYTHRIL_SKIP_PACMAN=1 to disable it explicitly — GitHub runners are
// Ubuntu and shouldn't attempt to ship an Arch package.
const SKIP_PACMAN = process.env.MYTHRIL_SKIP_PACMAN === '1';
const SKIP_CODE_SIGNING = process.env.MYTHRIL_SKIP_CODE_SIGNING === '1';
...
// ~/.mythril/engines/<agent>/<version>/ and the adapters are pointed at them via
...
module.exports = {
    ...
    packagerConfig: {
        executableName: 'mythril',
        icon: './icons/icon',  // .icns extension added automatically
        appBundleId: 'com.methalo11885260.mythril',
        appCategoryType: 'public.app-category.productivity',
        protocols: [
            { name: 'Mythril', schemes: ['mythril'] },
        ],
        extendInfo: {
            NSAudioCaptureUsageDescription: 'Mythril needs access to system audio to transcribe meetings from other apps (Zoom, Meet, etc.)',
            NSCameraUsageDescription: 'Mythril uses your camera in video chat mode so the assistant can see you and give feedback (e.g. pitch practice).',
        },
        // Signs the packaged app's executables (mythril.exe etc.); the Squirrel
    ...
    makers: [
        {
            ...
            config: (arch) => ({
                format: 'ULFO',
                name: `Mythril-darwin-${arch}-${pkg.version}`,  // Architecture-specific name to avoid conflicts
            })
        },
        {
            name: '@electron-forge/maker-squirrel',
            config: (arch) => ({
                authors: 'MetHalo11885260',
                description: 'Private multi-key NVIDIA NIM AI client',
                name: `Mythril-win32-${arch}`,
                setupExe: `Mythril-win32-${arch}-${pkg.version}-setup.exe`,
                ...
                iconUrl: 'https://raw.githubusercontent.com/MetHalo11885260/mythril/main/apps/x/apps/main/icons/icon.ico',
        ...
        },
        {
            ...
            config: (arch) => ({
                options: {
                    ...
                    bin: "mythril",
                    description: 'Private multi-key NVIDIA NIM AI client',
                    maintainer: 'MetHalo11885260',
                    homepage: 'https://MetHalo11885260.com',
                    icon: path.join(__dirname, 'icons/icon.png'),
                    mimeType: ['x-scheme-handler/mythril'],
                }
            })
        },
        {
        ...
        // Arch Linux package — local-only; disabled in CI via MYTHRIL_SKIP_PACMAN.
        ...(SKIP_PACMAN ? [] : [{
    ...
    hooks: {
        ...
        generateAssets: async (forgeConfig, platform, arch) => {
            ...
            // main imports it for the hosted mythril-server transport
