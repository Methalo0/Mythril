...
await esbuild.build({
  ...
  define: {
    ...
    'process.env.MYTHRIL_APP_VERSION': JSON.stringify(pkg.version ?? ''),
  },
});
...
// Bundle the standalone mythril-server (spawned as a child with
...
await esbuild.build({
  ...
  target: 'node20',
  format: 'cjs',
  outfile: './.package/dist/mythril-server.cjs',
...
console.log('✅ mythril-server bundled to .package/dist/mythril-server.cjs');
...
await esbuild.build({
  ...
  define: {
    ...
    // for a package.json and would find Mythril's instead of agent-slack's.
