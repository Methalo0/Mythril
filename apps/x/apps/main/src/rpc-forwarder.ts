...
// migrated to mythril-server are forwarded over real localhost HTTP instead
...
// MYTHRIL_FORWARD_MIGRATED=0 is the emergency kill switch.

export function forwardingEnabled(): boolean {
  const env = process.env.MYTHRIL_FORWARD_MIGRATED;
  if (env !== undefined) {
