...
const REPO_URL = "https://github.com/mythrillabs/mythril";
...
function navigate(type: string): void {
  actions?.openApp();
  dispatchDeepLink(`mythril://open?type=${type}`);
}
...
function rebuildMenu(): void {
  ...
  const macAppMenu: MenuItemConstructorOptions = {
    label: app.name,
    submenu: [
      { label: "About Mythril", click: () => showAboutDialog() },
      { type: "separator" },
      settingsItem,
      updateMenuItem(),
      { type: "separator" },
  ...
  const helpMenu: MenuItemConstructorOptions = {
    role: "help",
    submenu: [
      { label: "Mythril on GitHub", click: () => void shell.openExternal(REPO_URL) },
      ...
      // ~/.mythril — configs, caches, synced calendars; the place support
      ...
      ...(isMac
        ...
        : [
            { type: "separator" } as MenuItemConstructorOptions,
            updateMenuItem(),
            { label: "About Mythril", click: () => showAboutDialog() } as MenuItemConstructorOptions,
          ]),
    ],
  };
