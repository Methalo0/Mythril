...
// Ambient meeting detection must ignore Mythril's own mic use: meeting
...
import { classifySchedule, processMythrilInstruction } from '@x/core/dist/knowledge/inline_tasks.js';
...
import { getMythrilConfig } from '@x/core/dist/config/mythril.js';
...
export function registerIpcHandlers(handlers: InvokeHandlers) {
  ...
  for (const [channel, handler] of Object.entries(handlers) as [
    ...
    // Strangler-fig: channels migrated to mythril-server cross localhost HTTP
    // instead of calling their in-process handler (which stays in the map as
    // the MYTHRIL_FORWARD_MIGRATED=0 kill switch).
...
// Non-window consumers of workspace:didChange — today the mythril-server WS
...
// Exported for the mythril-server host, whose sessions:list handler shares
  ...
  registerIpcHandlers({
    ...
    'meeting:notifyNotesReady': async (_event, args) => {
      ...
      void notifyIfEnabled('meeting_notes_ready', {
        ...
        link: `mythril://open?type=file&path=${encodeURIComponent(args.notePath)}`,
    ...
    'models:getConfig': async () => {
      ...
      return {
        providers: Object.entries(cfg?.providers ?? {}).map(([id, entry]) => ({
          ...
          keyCount: [entry.apiKey, ...(entry.apiKeys ?? [])].filter(Boolean).length,
        })),
    ...
    'models:addProviderKey': async (_event, args) => {
      const repo = container.resolve<IModelConfigRepo>('modelConfigRepo');
      const keyCount = await repo.addProviderKey(args.id, args.apiKey);
      return { success: true as const, keyCount };
    },
    'models:removeProviderKey': async (_event, args) => {
      const repo = container.resolve<IModelConfigRepo>('modelConfigRepo');
      const keyCount = await repo.removeProviderKey(args.id, args.index);
      return { success: true as const, keyCount };
    },
    ...
    // getMythrilConfig caches once per app run; best-effort null on failure.
    'mythril:getConfig': async () => {
      return await getMythrilConfig().catch(() => null);
    },
    ...
    // Managed (mythril-mode) OAuth-redirect Picker: the Mythril backend runs the
    ...
    'inline-task:process': async (_event, args) => {
      return await processMythrilInstruction(args.instruction, args.noteContent, args.notePath);
    },
    ...
    // Mythril server (phone pairing) — client-local: answered by main, which
    // hosts the transport.
    ...
    'oauth:deliverLoopbackCallback': async () => {
      throw new Error('oauth:deliverLoopbackCallback is served by mythril-server');
    },
