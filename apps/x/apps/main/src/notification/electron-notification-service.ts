...
export class ElectronNotificationService implements INotificationService {
    ...
    notify({ title = "Mythril", message, link, actionLabel, secondaryActions, onlyWhenBackground, suppressDuringStartupGrace }: NotifyInput): void {
        ...
        const openLink = (target: string | undefined) => {
            if (target && MYTHRIL_URL.test(target)) {
                dispatchUrl(target);
