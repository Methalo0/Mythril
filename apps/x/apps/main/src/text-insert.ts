 ...
 * non-Mythril frontmost wins (the user is looking at it right now); the
 * stored capture covers the case where a Mythril window took focus in
 * between. Never pastes into Mythril itself.
 */
  ...
  async insert(text: string): Promise<TextInsertResult> {
    ...
    try {
      ...
      const friendly = /not allowed|1002|assistive|osascript is not allowed/i.test(msg)
        ? 'macOS blocked the keystroke — allow Mythril (in dev: "Electron") under Privacy & Security → Accessibility, and → Automation → System Events.'
        : `Paste failed: ${msg}`;
