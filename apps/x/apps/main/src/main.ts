...
// Single-instance lock: route a second launch (e.g. clicking a mythril:// link)
// back into the existing process via the 'second-instance' event.
if (app.isPackaged && !app.requestSingleInstanceLock()) {
  console.error('[Main] Another Mythril instance is already running; exiting this process.');
  app.quit();
  process.exit(0);
}

// Sandboxed dev instances (MYTHRIL_WORKDIR set, e.g. via `npm run dev:sandbox`)
...
if (!app.isPackaged && process.env.MYTHRIL_WORKDIR) {
  app.setPath('userData', path.join(WorkDir, '.electron-data'));
}

// Register as the OS handler for mythril:// URLs.
...
function createWindow(options: { startHidden?: boolean } = {}) {
  ...
  const openOrgLinkInApp = (url: string): boolean => {
    ...
    dispatchUrl(`mythril://open?${target.toString()}`);
    return true;
  };
...
app.whenReady().then(async () => {
  ...
  // Start the Mythril Apps server (per-app origins on 127.0.0.1:3210) BEFORE
  ...
  // Mythril: meetings removed — no mic monitor, no detection popup.
  void initMeetingPopup; void initMeetingDetection; void showMeetingPopup;
  void isRecordingActive;
  if (false) {
    initMeetingPopup({
    onTakeNotes: (meeting) => {
      showApp();
  ...
  } // end if (false) — meetings disabled in Mythril
  ...
  // In child/remote mode the watcher runs inside mythril-server and its
  ...
  // mythril-server transport (phone/API clients + the strangler-fig channel
  ...
  startServerHost().catch((error) => {
    console.error('[server-host] failed to start mythril-server:', error);
  });
  ...
  // headless-safe half of boot, shared with the standalone mythril-server
