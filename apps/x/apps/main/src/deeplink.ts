...
export const DEEP_LINK_SCHEME = "mythril";
...
/**
 * Dispatch any mythril:// URL — chooses among action / oauth-completion /
 * navigation automatically. Use this from notification click handlers and
 * other URL entry points.
 *
 * OAuth completion (mythril://oauth/google/done?session=<state>) is handled
...
// --- OAuth completion (mythril-mode Google connect) ---
...
/**
 * Match mythril://oauth/google/done?session=<state>. Returns null for
...
async function dispatchOAuthCompletion(url: string): Promise<void> {
    ...
    // oauthEvent IPC that completeMythrilGoogleConnect emits.
    const win = mainWindowRef;
    ...
    const { completeMythrilGoogleConnect } = await import("@x/core/dist/auth/oauth-flows.js");
    await completeMythrilGoogleConnect(parsed.state);
}
...
/**
 * Match mythril://oauth/google/picker/done?session=<state>. Distinct from the
