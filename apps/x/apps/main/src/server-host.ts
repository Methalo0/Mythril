...
// Phase 7a (SEPARATION_PLAN.md): with MYTHRIL_CHILD_SERVER=1, main does not
// host the transport in-process — it spawns the standalone mythril-server as
...
// Phase 8: MYTHRIL_REMOTE_SERVER=<url> (+ MYTHRIL_REMOTE_TOKEN) points the
// desktop at a mythril-server on another machine — no child is spawned, no
...
function remoteTarget(): { baseUrl: string; key: string; fromEnv: boolean } | null {
  if (process.env.MYTHRIL_REMOTE_SERVER) {
    return {
      baseUrl: process.env.MYTHRIL_REMOTE_SERVER.replace(/\/+$/, ''),
      key: (process.env.MYTHRIL_REMOTE_TOKEN ?? '').trim(),
      fromEnv: true,
    };
  }
...
export function serverHostMode(): ServerHostMode {
  ...
  // Mythril: run core in-process by default — the child server doubles
  // baseline RAM and puts an HTTP+WS hop between every user action and the
  // runtime. Set MYTHRIL_CHILD_SERVER=1 to bring the split back.
  const env = process.env.MYTHRIL_CHILD_SERVER;
  if (env !== undefined && (env === '1' || env.toLowerCase() === 'true')) return 'child';
  return 'in-process';
}
        ...
        const { server, port } = await createRelayAuthServer(
          ...
          async (callbackUrl) => {
            ...
            const body = (await res.json().catch(() => null)) as
              ...
              | null;
            if (!res.ok || !body) {
              return { accepted: false, message: 'Could not reach the Mythril server to complete sign-in' };
            }
...
async function isMythrilServer(baseUrl: string): Promise<boolean> {
  try {
    const res = await fetch(`${baseUrl}/health`, { signal: AbortSignal.timeout(2500) });
    const body = (await res.json()) as { name?: string };
    return body.name === 'mythril-server';
  } catch {
    return false;
  }
}
...
async function connectRemote(): Promise<RemoteServer> {
  ...
  while (!(await isMythrilServer(baseUrl))) {
    if (Date.now() > deadline) {
      throw new Error(`no mythril-server reachable at ${baseUrl} within 20s`);
    }
    await new Promise((r) => setTimeout(r, 1000));
  }
  const events = createDesktopEventsClient(baseUrl, key);
  console.log(`[server-host] connected to remote mythril-server at ${baseUrl}`);
...
async function launchChild(): Promise<ChildServer> {
  ...
  const entry =
    process.env.MYTHRIL_SERVER_ENTRY ??
    (app.isPackaged
      ? path.join(__dirname, 'mythril-server.cjs')
      : path.resolve(app.getAppPath(), '../server/dist/standalone.js'));
  const child = spawn(process.execPath, [entry], {
    env: { ...process.env, ELECTRON_RUN_AS_NODE: '1', MYTHRIL_PARENT_PID: String(process.pid) },
    stdio: ['ignore', 'inherit', 'inherit'],
  });
  child.on('exit', (code) => {
    console.error(`[server-host] child mythril-server exited (code ${code})`);
  });
  ...
  for (;;) {
    try {
      ...
      const body = (await res.json()) as { name?: string };
      if (body.name === 'mythril-server') break;
    } catch {
      // not up yet
    }
    if (Date.now() > deadline) {
      child.kill();
      throw new Error('child mythril-server did not become healthy within 60s');
    }
...
// Vertical-slice hosting: main runs the mythril-server transport in-process
...
type HostedServer = MythrilServer | ChildServer | RemoteServer;

let current: HostedServer | null = null;
let ready: Promise<HostedServer> | null = null;

async function launchInProcess(): Promise<MythrilServer> {
  const server = await createMythrilServer({
    workDir: WorkDir,
  ...
  current = server;
  console.log(`[server-host] mythril-server on http://${server.host}:${server.port} (lan: ${server.lanEnabled})`);
  return server;
}
...
export async function setLanEnabled(enabled: boolean): Promise<void> {
  if (serverHostMode() === 'remote') {
    throw new Error('Not available while connected to a remote mythril-server');
  }
  const config = await loadServerConfig(WorkDir);
...
export async function connectRemoteServer(url: string, token: string): Promise<{ success: boolean; error?: string }> {
  ...
  if (serverHostMode() === 'in-process') {
    return { success: false, error: 'Not available while MYTHRIL_CHILD_SERVER=0 is set.' };
  }
  ...
  if (!(await isMythrilServer(baseUrl))) {
    return { success: false, error: 'No Mythril server answered at that address' };
  }
  try {
