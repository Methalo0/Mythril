...
// OAuth loopback relay (Phase 8b): with a remote mythril-server, provider
...
export async function deliverLoopbackCallback(args: {
  ...
  if (!entry) {
    return { accepted: false, message: 'This sign-in attempt is no longer active. Close this tab and retry from Mythril.' };
  }
  let url: URL;
  try {
