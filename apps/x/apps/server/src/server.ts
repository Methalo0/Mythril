...
export interface MythrilServerOptions {
  workDir: string;
...
export interface MythrilServer {
  port: number;
  host: string;
  lanEnabled: boolean;
  key: string;
...
/** Is the process answering on this port another mythril-server? */
async function isMythrilServer(port: number): Promise<boolean> {
  try {
...
export async function createMythrilServer(opts: MythrilServerOptions): Promise<MythrilServer> {
  ...
  try {
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        // Colliding with another mythril-server means two hosts are live —
        // that must fail loudly, never quietly bind one port over.
        if (tryPort !== 0 && (await isMythrilServer(tryPort))) {
          throw new Error(
            `another mythril-server is already listening on port ${tryPort} — refusing to start a second instance`,
          );
        }
