...
import { createMythrilServer } from './server.js';
...
// Headless mythril-server: the RFC's end-state entrypoint, where main spawns
...
// Electron app is using — two core instances over one ~/.mythril double-run
...
// isolated MYTHRIL_WORKDIR.
//

async function main(): Promise<void> {
  // The workdir lock is acquired by createMythrilServer itself — a live
  ...
  console.log(`[server] mythril-server listening on http://${server.host}:${server.port} (workdir: ${WorkDir})`);
  ...
  const parentPid = Number(process.env.MYTHRIL_PARENT_PID ?? '');
  if (parentPid > 0) {
    setInterval(() => {
      try {
