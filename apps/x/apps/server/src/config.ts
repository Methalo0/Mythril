...
// ~/.mythril/config/server.json — user-facing knobs for the transport.
// 3210 is taken by the Mythril Apps server; 3220 is ours.
...
// lets several mythril-servers coexist on one machine, each on its own port,
// without touching the workdir's server.json. Explicit opts.port (tests)
// still wins over this — see createMythrilServer.
function envPortOverride(): number | undefined {
  const port = Number(process.env.MYTHRIL_SERVER_PORT);
