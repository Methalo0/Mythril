...
describe('events client ↔ mythril-server', () => {
  let workDir: string;
  ...
  beforeAll(async () => {
    workDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-events-client-'));
...
describe('reverse calls (capability requests)', () => {
  it('routes a request to a capable client and returns its reply', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-cap-'));
