...
import { createMythrilServer, type EventSources, type MythrilServer } from './server.js';
...
describe('mythril-server transport', () => {
  let workDir: string;
  let server: MythrilServer;
  let base: string;
  ...
  it('serves /health unauthenticated', async () => {
    ...
    expect(res.headers.get('x-mythril-api-version')).toBe('0');
  });
  ...
  it('drops clients that never say hello', async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-hello-test-'));
    const quick = await createMythrilServer({
      workDir: tmp,
      handlers: stubHandlers({}),
      events,
...
describe('hardening guards', () => {
  ...
  it('rejects requests with a foreign Host header (DNS rebinding)', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-host-'));
  ...
  it('refuses to serve a symlink that escapes the workspace', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-symlink-'));
    const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-outside-'));
  ...
  it('refuses a workdir whose lock is held by a live foreign process', async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-lock-'));
  ...
  it('fails loudly when the port is held by another mythril-server', async () => {
    const dirA = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-portA-'));
    const dirB = await fs.mkdtemp(path.join(os.tmpdir(), 'mythril-portB-'));
