...
// production app, and from inside Mythril code mode (whose agent shells leak
...
//
//   --seed-config   copy ~/.mythril/config into the sandbox workdir on first
//                   boot (minus server.json), so model providers work
//   --no-deps       skip the workspace deps build (shared/core/server/…)
//   --workdir       explicit workdir (default ~/.mythril-dev/<instance-id>)
...
const workDir = path.resolve(flagValue('--workdir') ?? path.join(os.homedir(), '.mythril-dev', instanceId));
...
if (hasFlag('--seed-config')) {
    const sourceConfigDir = path.join(os.homedir(), '.mythril', 'config');
    if (fs.existsSync(seededConfigDir)) {
        ...
        // workdir, and the sandbox's port comes from MYTHRIL_SERVER_PORT.
...
env.MYTHRIL_WORKDIR = workDir;
env.MYTHRIL_SERVER_PORT = String(serverPort);
env.MYTHRIL_APPS_PORT = String(appsPort);
env.MYTHRIL_DEV_SERVER_URL = `http://localhost:${vitePort}`;
