...
export interface RegistryClient {
    ...
    latestManifest(record: RegistryRecord): Promise<MythrilAppManifest>;
}
...
export class GitHubRegistryClient implements RegistryClient {
     ...
     */
    async latestManifest(record: RegistryRecord): Promise<MythrilAppManifest> {
        const res = await fetch(`https://github.com/${record.repo}/releases/latest/download/mythril-app.json`, {
            redirect: 'follow',
        });
        if (!res.ok) throw new Error(`latest_manifest_unavailable: HTTP ${res.status}`);
        const manifest = MythrilAppManifestSchema.parse(await res.json());
