...
// Packager (spec §4.4): builds the `<name>.mythril-app` ZIP from the ALLOWLIST
// only — mythril-app.json, dist/**, agents/<manifest.agents>, defaults/** — in
// sorted path order (determinism). Personal data cannot leak by omission (D15):
// src/, package.json, node_modules/, data/, dotfiles, and .mythril-*.json are
...
/**
 * Build `<name>.mythril-app` for the app at `folder`, writing the bundle to
...
export async function packageApp(folder: string, outDir: string): Promise<PackageResult> {
    ...
    try {
        manifest = MythrilAppManifestSchema.parse(
            JSON.parse(await fsp.readFile(path.join(dir, 'mythril-app.json'), 'utf-8')),
        );
    } catch (e) {
        throw new PackageError('invalid_manifest', `mythril-app.json is missing or invalid: ${e instanceof Error ? e.message : String(e)}`);
    }
    ...
    const files: string[] = ['mythril-app.json'];
    ...
    const bundlePath = path.join(outDir, `${manifest.name}.mythril-app`);
