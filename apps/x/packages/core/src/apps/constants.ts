...
// Mythril Apps constants (spec §3). All apps constants live here; values the
// renderer needs are mirrored through IPC responses, never imported directly.

export const APPS_DIR = path.join(WorkDir, 'apps');

// 3210 reuses the local-sites port (D8). MYTHRIL_APPS_PORT is the
...
const envAppsPort = Number(process.env.MYTHRIL_APPS_PORT);
...
export const REGISTRY_REPO = process.env.MYTHRIL_APPS_REGISTRY || 'mythrillabs/apps-registry';
