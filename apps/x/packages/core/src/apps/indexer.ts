...
async function summarizeApp(folder: string): Promise<AppSummary | null> {
    ...
    const manifestPath = path.join(dir, 'mythril-app.json');
    ...
    let manifest: MythrilAppManifest | undefined;
    let manifestError: string | undefined;
    try {
        const parsed = MythrilAppManifestSchema.safeParse(JSON.parse(manifestRaw));
        if (parsed.success) {
    ...
    const installRaw = await readJsonIfExists(path.join(dir, '.mythril-install.json'));
    const install = installRaw !== undefined ? AppInstallRecordSchema.safeParse(installRaw) : undefined;
    const publishRaw = await readJsonIfExists(path.join(dir, '.mythril-publish.json'));
...
export async function listApps(): Promise<AppSummary[]> {
    ...
    for (const entry of entries) {
        ...
        // app folder (a dev repo linked into ~/.mythril/apps) was skipped here
...
<body>
  ...
  <script>
    fetch('/_mythril/app').then(function (r) { return r.json(); }).then(function (a) {
...
export async function createApp(input: { folder: string; name: string; description: string }): Promise<AppSummary> {
    ...
    const manifest = MythrilAppManifestSchema.parse({
        schemaVersion: 1,
        name,
        version: '0.1.0',
        description,
    ...
    await fs.writeFile(path.join(dir, 'mythril-app.json'), JSON.stringify(manifest, null, 2) + '\n');
...
export async function deleteApp(folder: string): Promise<void> {
    ...
    const dir = appDir(folder);
    try {
        await fs.access(path.join(dir, '.mythril-install.json'));
