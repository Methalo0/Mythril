...
import { MythrilAppManifestSchema, type MythrilAppManifest } from '@x/shared/dist/mythril-app.js';
import {
    APPS_DIR,
    APPS_PORT,
    APPS_HOST_SUFFIX,
...
// Mythril Apps server (spec §6–§7). Adapted from the deleted local-sites
...
// app's dist/ and the same-origin Host API under /_mythril/*.
...
function loadManifest(slug: string): { manifest?: MythrilAppManifest; error?: string } {
    try {
        const raw = fs.readFileSync(path.join(appDirFor(slug), 'mythril-app.json'), 'utf-8');
        const parsed = MythrilAppManifestSchema.safeParse(JSON.parse(raw));
        if (!parsed.success) {
...
(() => {
  ...
  const connect = () => {
    ...
    source = new EventSource(new URL('/_mythril/events', window.location.origin).toString());
    source.addEventListener('message', (event) => {
      try {
        ...
        if (payload.area === 'data') {
          ...
          const domEvent = new CustomEvent('mythril:data-change', { cancelable: true, detail: { path: payload.path } });
  ...
  if (params.get('__mythril_embed') !== '1') return;
  ...
  const publishHeight = () => {
    ...
    window.parent.postMessage({ type: 'mythril:iframe-height', height: nextHeight, href: window.location.href }, '*');
  };
...
function contractFor(manifest: MythrilAppManifest, relPath: string) {
...
async function handleDataApi(
    ...
    // GET /_mythril/data?list=<dir> — non-recursive listing.
    if (pathname === '/_mythril/data' && req.method === 'GET') {
    ...
    // File operations: /_mythril/data/<path>
    const relRaw = pathname.slice('/_mythril/data/'.length);
    let rel: string;
    try {
...
/** Register an additional POST /_mythril/<name> endpoint (used by M2 wiring). */
...
async function handleHostApi(
    ...
    if (req.method !== 'GET' && req.method !== 'HEAD') {
        if (req.headers['x-mythril-app'] === undefined) {
            return sendError(res, 403, 'missing_app_header', 'non-GET /_mythril requests must set X-Mythril-App: 1');
        }
    ...
    if (pathname === '/_mythril/app' && req.method === 'GET') {
        res.json({
            name: manifest.name,
    ...
    if (pathname === '/_mythril/events' && req.method === 'GET') {
        handleEventsRequest(slug, req, res);
        return;
    }

    if (pathname === '/_mythril/data' || pathname.startsWith('/_mythril/data/')) {
...
function createApp(): express.Express {
    ...
    appServer.use((req, res) => {
        void (async () => {
            ...
            if (!manifest) {
                html503(res, 'Invalid app', `“${slug}” has a missing or invalid mythril-app.json: ${error ?? 'unknown error'}`);
                return;
            }

            if (pathname === '/_mythril' || pathname.startsWith('/_mythril/')) {
...
async function startWatcher(): Promise<void> {
    ...
    w.on('all', (eventName, absolutePath) => {
        ...
        // a full page reload — README.md, .mythril-install.json,
        // .mythril-publish.json (rewritten at every publish step), agents/,
        ...
        else if (top === 'dist' || hit.rel === 'mythril-app.json') area = 'dist';
        else return;
...
export async function init(): Promise<void> {
    ...
    startPromise = (async () => {
        try {
            ...
            // EADDRINUSE almost always means a previous Mythril instance is
