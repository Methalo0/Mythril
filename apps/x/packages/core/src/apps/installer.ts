...
const RELEASE_MANAGED = ['mythril-app.json', 'dist', 'agents', 'defaults'];
...
async function assembleInstall(
    ...
    try {
        ...
        await fsp.writeFile(
            path.join(dir, '.mythril-install.json'),
...
function validatePkgManifest(pkgDir: string, expected?: { name: string; version: string }): MythrilAppManifest {
    let manifest: MythrilAppManifest;
    try {
        manifest = MythrilAppManifestSchema.parse(
            JSON.parse(fs.readFileSync(path.join(pkgDir, 'mythril-app.json'), 'utf-8')),
        );
    } catch (e) {
...
function checkCapabilitySubset(bundle: MythrilAppManifest, confirmed: { capabilities: string[]; agents: string[] }): void {
...
export async function installFromRegistry(record: RegistryRecord, confirmed: InstallPreview): Promise<InstallDone> {
    ...
    try {
        const bundleUrl = `https://github.com/${record.repo}/releases/latest/download/${record.name}.mythril-app`;
...
async function readInstallRecord(folder: string): Promise<AppInstallRecord> {
    try {
        return AppInstallRecordSchema.parse(
            JSON.parse(await fsp.readFile(path.join(APPS_DIR, folder, '.mythril-install.json'), 'utf-8')),
        );
    } catch {
...
export async function updateApp(
    ...
    try {
        const bundleUrl = `https://github.com/${install.repo}/releases/latest/download/${install.name}.mythril-app`;
        const { zipPath, sha256 } = await downloadBundle(bundleUrl, staging);
        const pkgDir = path.join(staging, 'pkg');
        const files = await extractBundle(zipPath, pkgDir);
        ...
        await fsp.writeFile(path.join(dir, '.mythril-install.json'), JSON.stringify(nextRecord, null, 2));
