 ...
 * to the native mythril-mode Google OAuth flow.
 *
 * Triggered by the renderer on app launch and after Mythril sign-in. The
...
export async function qualifyAndDisconnectComposioGoogle(): Promise<{ shouldShow: boolean }> {
    ...
    // Rule 1 — must be signed in to Mythril
    if (!(await isSignedIn())) {
        return { shouldShow: false };
    }

    // Rule 3 — already on native mythril-mode Google → silently mark dismissed
    ...
    if (googleConnection.tokens && googleConnection.mode === 'mythril') {
        markDismissed();
        return { shouldShow: false };
    }
