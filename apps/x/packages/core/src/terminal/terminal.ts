...
// mythril-server relays to WS clients (broadcast-to-all, per RFC Q12/Q13).
...
// (the mythril-server child that now serves terminal calls) while the CJS
...
function spawnEntry(id: string, cwd: string, cols: number, rows: number): TerminalEntry {
  ...
  try {
    proc = pty.spawn(file, args, {
      ...
      cols,
      rows,
      env: { ...process.env, TERM_PROGRAM: 'mythril' } as Record<string, string>,
    });
  } catch (err) {
