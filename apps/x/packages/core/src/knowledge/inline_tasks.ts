...
/**
 * Parse the tell-mythril block content (JSON format).
 * Returns { instruction, schedule } or null if not valid JSON.
 * Also supports legacy @mythril format.
 */
function parseBlockContent(contentLines: string[]): { instruction: string; schedule: InlineTaskSchedule | null; lastRunAt: string | null; targetId: string | null } | null {
    ...
    try {
        ...
        // Legacy format: @mythril lines + optional schedule: JSON line
    }

    // Legacy fallback: parse @mythril instruction and schedule: line
    ...
    const firstMythrilLine = instructionLines.find(l => l.trim().startsWith('@mythril'));
    const rawInstruction = firstMythrilLine?.trim() ?? instructionLines.join('\n').trim();
    const instruction = rawInstruction.replace(/^@mythril:?\s*/, '');
...
/**
 * Find ```tell-mythril code blocks in a note body and return tasks that are pending execution.
 */
function findPendingTasks(body: string): InlineTask[] {
    ...
    while (i < lines.length) {
        ...
        if (trimmed.startsWith('```task') || trimmed.startsWith('```tell-mythril')) {
            const startLine = i;
            i++;
...
/**
 * Insert the agent result below the tell-mythril code block in the body.
...
/**
 * Determine if a note has any "live" tell-mythril tasks.
 * A task is live if:
    ...
    for (const filePath of allFiles) {
        ...
        if (tasks.length === 0) {
            ...
            if (!live) {
                ...
                // Remove mythril_tasks if present (legacy cleanup)
                delete fields['mythril_tasks'];
        ...
        // Update frontmatter — only manage live_note, remove legacy mythril_tasks
...
/**
 * Process a @mythril instruction via the inline task agent.
...
export async function processMythrilInstruction(
    instruction: string,
    noteContent: string,
    notePath: string,
): Promise<{
    ...
    const message = [
        `Process the following @mythril instruction from the note "${notePath}":`,
        '',
