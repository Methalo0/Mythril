 ...
 *   - `mythril` signed-in user; client_id+secret never on the desktop;
 ...
 *
 * **Auto-refresh disabled in mythril mode:** google-auth-library's
 * OAuth2Client will, on a 401 from a Google API call, try to refresh using
 * the refresh_token + client secret it has on hand. In mythril mode we have
...
export class GoogleClientFactory {
    ...
    private static async getClientInner(): Promise<OAuth2Client | null> {
        ...
        // BYOK needs an openid-client Configuration for local refresh; mythril doesn't.
        if (mode === 'byok') {
            try {
    ...
    private static async refreshAndBuild(tokens: OAuthTokens, mode: Mode): Promise<OAuth2Client | null> {
        ...
        try {
            ...
            // Walk cause chain so we can see e.g. `Not signed into Mythril`
    ...
    private static async buildAndCacheClient(tokens: OAuthTokens, mode: Mode): Promise<OAuth2Client> {
        ...
        const client = mode === 'mythril'
            ? this.createMythrilClient(tokens)
    ...
    /**
     * Initialize cached configuration for BYOK mode (mythril doesn't need it).
     */
    ...
    /**
     * Mythril OAuth2Client — no client_id/secret, no refresh_token.
     ...
     */
    private static createMythrilClient(tokens: OAuthTokens): OAuth2Client {
