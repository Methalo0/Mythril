 ...
 * Handled by the host's `mythril://open` router; shared by the scheduler's
...
export function spaceLink(orgId: string, spaceId: string, threadRootId?: string, messageId?: string): string {
  ...
  return `mythril://open?type=spaces&orgId=${encodeURIComponent(orgId)}&spaceId=${encodeURIComponent(spaceId)}${thread}${message}`;
}
