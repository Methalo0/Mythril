...
// Managed orgs ride the Mythril account session (one session, two uses —
// 2026-09-14). Under test: the registry's handling of `session` records and
// of the pre-`session` `oauth` records whose issuer IS the Mythril desk —
...
const legacyManaged: OrgRecord = {
    id: 'org-legacy',
    name: 'Mythril',
    address: 'mythril.spaces.example',
    baseUrl: 'https://mythril.spaces.example',
    auth: {
        kind: 'oauth',
...
describe('applyManagedListing', () => {
    it('rewrites a legacy managed record in place, adds new ones, drops session records the apex no longer lists, and leaves foreign + dev alone', () => {
        orgs.applyManagedListing(
            [
                { id: 'org-01MYTHRIL', name: 'Mythril Labs', address: 'mythril.spaces.example', memberId: 'm-new' },
    ...
    it('is idempotent: applying the same listing twice changes nothing', () => {
        const listing = [{ id: 'org-01MYTHRIL', name: 'Mythril', address: 'mythril.spaces.example', memberId: 'm-new' }];
...
describe('deriveSpacesMcpServers with a session', () => {
    it('gives session-backed orgs the session bearer and foreign orgs their own token', () => {
        ...
        expect(entries['spaces-mythril']!.headers.authorization).toBe('Bearer session-access');
