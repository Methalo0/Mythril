...
// @mythril in a space (spec §8 grammar, §11 beat 7): an addressed message
...
//   room's "Mythril is working" chip and lease. The session is user-openable
...
export interface InvokeTopicAgentInput {
  ...
  threadLabel: string;
  spaceName: string;
  /** Feed message id of the @mythril message — the invocation's provenance. */
  messageId: string;
  /** The message body, verbatim (the @mythril address included). */
  body: string;
...
export function buildInvocationMessage(input: InvokeTopicAgentInput, mcpServerName: string | null): string {
  ...
  return [
    `[@mythril in "${input.spaceName}" · spaceId ${input.spaceId} · thread ${input.threadRootId} · message ${input.messageId}]`,
    input.body,
  ].join('\n');
}
