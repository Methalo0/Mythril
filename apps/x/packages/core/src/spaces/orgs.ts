...
import type { ServerFrame } from '@mythril/spaces-protocol';
...
//
//   session  a MANAGED org (one on the Mythril deployment): the org trusts
//            the same login desk the Mythril account comes from, so the
...
//            An `oauth` record whose issuer IS the Mythril desk — written by
...
// org (spec §4). The Mythril session's own refresh lives in auth/tokens.ts.
...
export type OrgAuth =
  ...
  | {
      /** Managed org: borrows the Mythril account session (auth/tokens.ts). */
      kind: 'session';
      issuer: string;
      memberId: string;
    }
...
export interface OrgRecord {
  ...
  id: string;
  name: string;
  /** The org address links are minted on, e.g. localhost:4272 or acme.mythril.space. */
  address: string;
...
/**
 * The login desk the Mythril account comes from — `<supabaseUrl>/auth/v1`,
...
export async function managedIssuer(): Promise<string | null> {
  ...
  try {
    const config = await getMythrilConfig();
...
/**
 * Does this org ride the Mythril session? `session` records by definition;
 ...
 * with no Mythril session (signed out, or a pre-`session` record after an
...
export async function describeOrgAuth(record: OrgRecord): Promise<{
  ...
  if (await isSessionBacked(record.auth)) {
    ...
    if (!session) return { authKind: 'session', authError: 'Sign in with your Mythril account' };
...
/** No Mythril session (signed out): the managed orgs that borrowed it go too. */
