...
describe('deriveSpacesMcpServers', () => {
    ...
    it('same org under two identities yields two correctly-credentialed entries (never an overwrite)', () => {
        ...
        expect(Object.keys(entries).sort()).toEqual([
            'spaces-mythril-labs-dev',
            'spaces-mythril-labs-dev-gagan',
        ]);
        expect(entries['spaces-mythril-labs-dev']!.headers.authorization).toBe('Bearer dev-ramnique');
        expect(entries['spaces-mythril-labs-dev-gagan']!.headers.authorization).toBe('Bearer dev-gagan');
    });
    ...
    it('falls back to the org id when the name slugs to nothing, and on a full name+member collision', () => {
        ...
        expect(Object.keys(collided).sort()).toEqual([
            'spaces-org-3',
            'spaces-mythril-labs-dev',
            'spaces-mythril-labs-dev-ramnique',
        ]);
    });
