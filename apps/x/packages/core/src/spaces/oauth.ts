...
import { ensureMythrilSession } from '../auth/oauth-flows.js';
...
import type { AcceptInviteResult, ResolveInviteResult } from '@mythril/spaces-protocol';
...
//
//   MANAGED — the org trusts the Mythril login desk, the same one the
//   Mythril account comes from. No dance of our own: the account session is
//   the identity (auth/tokens.ts, one session two uses). If no session
//   exists yet, the ordinary Mythril sign-in runs once and the session is
...
export async function danceForTokens(input: { baseUrl: string; openBrowser: OpenBrowser }): Promise<DanceResult> {
  ...
  try {
    ...
    try {
      ...
      let clientId: string;
      try {
        const registered = await oauthClient.registerClient(issuer, [pending.redirectUri], SCOPES, 'Mythril');
      ...
      res
        ...
        .end(
          denied
            ...
            : `<p style="font-family:system-ui;margin:3rem">You're signed in — return to Mythril.</p>`,
        );
...
/** Is this issuer the Mythril login desk — i.e. does the org ride the account session? */
 ...
 * session (signing in to Mythril first if there is none), foreign ones
 ...
 * MYTHRIL_SPACES_APEX (dev/local-stack override) → the api's /v1/config
...
export async function apexUrl(): Promise<string> {
  const override = process.env.MYTHRIL_SPACES_APEX;
 ...
 * the identity (the apex trusts the Mythril login desk; a local-stack apex
...
export async function createOrgOnDeployment(input: {
  ...
  if (managed) await ensureMythrilSession();
 ...
 * signing in to Mythril once if there is none; this install's own dance for
...
export async function joinViaInviteLink(input: {
  ...
  if (existing) {
    ...
    if (usable) {
      if (sessionBacked) await ensureMythrilSession();
...
/** What the Spaces UI needs to know about the Mythril session (auth/tokens.ts): is there one, and is the app signed in on it? */
...
/**
 * The Spaces door's "Sign in with Mythril": a session (browser sign-in if
