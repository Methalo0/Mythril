...
const input = {
    ...
    body: '@mythril move SSO to P1',
};

describe('buildInvocationMessage', () => {
    it('is a one-line header (space, ids) plus the verbatim ask — nothing else', () => {
        const msg = buildInvocationMessage(input, 'spaces-mythril-labs-dev');
        const lines = msg.split('\n');
        expect(lines).toHaveLength(2);
        expect(lines[0]).toBe(
            '[@mythril in "Roadboard" · spaceId 01M07B68G1BQFP70TX5RPHJX89 · thread 01M07ROOTAAAAAAAAAAAAAAAA1 · message 01M07MSGAAAAAAAAAAAAAAAAA1]',
        );
        expect(lines[1]).toBe('@mythril move SSO to P1');
    });
    ...
    it('does not depend on the server name (the org rides the session pin)', () => {
        expect(buildInvocationMessage(input, null)).toBe(buildInvocationMessage(input, 'spaces-mythril-labs-dev'));
    });
});
