...
const APP_VERSION = (process.env.MYTHRIL_APP_VERSION ?? process.env.npm_package_version ?? '').trim();
...
export function identify(userId: string, properties?: Record<string, unknown>): void {
  ...
  if (!ph) return;
  try {
    // Alias the anonymous installation ID to the mythril user ID so historical
...
/**
 * Merge person properties onto the CURRENT identity — the mythril user once
...
/**
 * Evaluate a PostHog feature flag for the current identity (mythril user id
