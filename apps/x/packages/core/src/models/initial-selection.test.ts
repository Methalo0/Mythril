...
describe('selectInitialModel', () => {
    ...
    it('accepts the nested { assistantModel, taskModels } wire shape', () => {
        const nested = { mythril: { assistantModel: 'google/gemini-3.5-flash', taskModels: {} } };
        expect(selectInitialModel('mythril', ['a', 'google/gemini-3.5-flash'], nested))
            .toEqual({ model: 'google/gemini-3.5-flash' });
    });

    it('carries effort from the { model, effort } wire shape', () => {
        const nested = { mythril: { assistantModel: { model: 'google/gemini-3.5-flash', effort: 'high' as const } } };
        expect(selectInitialModel('mythril', ['google/gemini-3.5-flash'], nested))
...
describe('selectInitialTaskModels', () => {
    ...
    it('writes overrides only for listed recs that differ from the assistant', () => {
        expect(selectInitialTaskModels('mythril', 'mythril', gatewayList, nested, { model: 'google/gemini-3.5-flash' }))
            .toEqual({
                knowledgeGraph: { provider: 'mythril', model: 'google/gemini-3.1-flash-lite' },
                chatTitle: { provider: 'mythril', model: 'google/gemini-3.5-flash-lite' },
            });
    });

    it('returns nothing for legacy flat recommendations or absent maps', () => {
        expect(selectInitialTaskModels('mythril', 'mythril', gatewayList, { mythril: 'google/gemini-3.5-flash' }, { model: 'x' }))
            .toEqual({});
        expect(selectInitialTaskModels('mythril', 'mythril', gatewayList, undefined, { model: 'x' })).toEqual({});
    });
    ...
    it('keeps a same-model rec whose effort differs from the assistant, and carries effort', () => {
        ...
        expect(selectInitialTaskModels('mythril', 'mythril', gatewayList, withEffort, { model: 'google/gemini-3.5-flash', effort: 'high' }))
            .toEqual({
                chatTitle: { provider: 'mythril', model: 'google/gemini-3.5-flash', effort: 'low' },
            });
    });
});
