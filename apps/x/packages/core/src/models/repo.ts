...
export interface IModelConfigRepo {
    ...
    /** Append a key to a provider's rotation pool. Returns the new key count. */
    addProviderKey(id: string, apiKey: string): Promise<number>;
    /** Remove the key at index (0 = primary). Returns the new key count. */
    removeProviderKey(id: string, index: number): Promise<number>;
    /**
...
export class FSModelConfigRepo implements IModelConfigRepo {
    ...
    async setProvider(id: string, provider: z.infer<typeof LlmProvider>): Promise<void> {
        ...
        if (provider.flavor === "mythril" || provider.flavor === "codex") {
    ...
    async addProviderKey(id: string, apiKey: string): Promise<number> {
        const config = await this.read();
        const entry = config.providers[id];
        if (!entry) throw new Error(`Provider '${id}' not found`);
        const keys = [entry.apiKey, ...(entry.apiKeys ?? [])].filter((k): k is string => !!k);
        if (!keys.includes(apiKey)) keys.push(apiKey);
        entry.apiKey = keys[0];
        entry.apiKeys = keys.slice(1);
        config.providers[id] = entry;
        await this.write(config);
        return keys.length;
    }

    async removeProviderKey(id: string, index: number): Promise<number> {
        const config = await this.read();
        const entry = config.providers[id];
        if (!entry) throw new Error(`Provider '${id}' not found`);
        const keys = [entry.apiKey, ...(entry.apiKeys ?? [])].filter((k): k is string => !!k);
        if (keys.length <= 1) throw new Error("A provider needs at least one key — disconnect it instead");
        keys.splice(index, 1);
        entry.apiKey = keys[0];
        entry.apiKeys = keys.slice(1);
        config.providers[id] = entry;
