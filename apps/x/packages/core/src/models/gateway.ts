...
// to the Mythril OpenRouter provider.
export const authedFetch: typeof fetch = async (input, init) => {
    const token = await getAccessToken();
    const headers = new Headers(init?.headers);
    headers.set('Authorization', `Bearer ${token}`);
    const ctx = getCurrentUseCase();
    if (ctx?.useCase) headers.set('x-mythril-use-case', ctx.useCase);
    if (ctx?.subUseCase) headers.set('x-mythril-sub-use-case', ctx.subUseCase);
    if (ctx?.agentName) headers.set('x-mythril-agent-name', ctx.agentName);
