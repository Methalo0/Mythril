...
const ANTHROPIC_BUDGET = { medium: 8192, high: 16384, xhigh: 32768, ultrahigh: 65536 } as const;
const GEMINI_25_BUDGET = { low: 2048, medium: 8192, high: 16384, xhigh: 32768, ultrahigh: 65536 } as const;
 ...
 * (openrouter, mythril) are forgiving — OpenRouter drops the field for
...
export function mapReasoningEffort(
    ...
    switch (flavor) {
        case "openai": {
            ...
            // OpenAI tops out at "high" — clamp the extended levels.
            const clamped = effort === "xhigh" || effort === "ultrahigh" ? "high" : effort;
            return { providerOptions: { openai: { reasoningEffort: clamped } } };
        }
        case "anthropic": {
            if (supportsReasoning !== true) return undefined;
            if (effort === "low") {
        ...
        case "google": {
            ...
            if (id.includes("gemini-3")) {
                ...
                // default is already deep, so "medium" sends nothing. The
                // extended levels clamp to "high".
                if (id.includes("pro") && effort === "medium") return undefined;
                const level = effort === "xhigh" || effort === "ultrahigh" ? "high" : effort;
                return {
                    providerOptions: {
        ...
        case "nvidia": {
            // NIM is OpenAI-compatible; send the effort through the
            // provider's own options namespace. Forgiving like OpenRouter —
            // models that can't reason ignore the field.
            if (supportsReasoning === false) return undefined;
            return { providerOptions: { nvidia: { reasoningEffort: effort } } };
        }
        case "codex": {
