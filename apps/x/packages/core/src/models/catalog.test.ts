...
/**
 * The unified model catalog: every provider (mythril gateway, codex, BYOK,
...
const mocks = vi.hoisted(() => ({
  ...
  listGatewayModels: vi.fn(async () => ({
    providers: [{ id: 'mythril', name: 'Mythril', models: [{ id: 'google/gemini-3.5-flash', reasoning: true }] }],
  })),
...
describe('getModelCatalog', () => {
  it('treats mythril, codex, and BYOK providers as one uniform provider list', async () => {
    ...
    expect(catalog.providers.map((p) => p.id)).toEqual(['mythril', 'codex', 'ollama']);
...
describe('getImageModelCatalog', () => {
  it('lists every image flavor from its own source — never a typed id', async () => {
    ...
    expect(catalog.providers.map((p) => p.id)).toEqual(['mythril', 'openrouter', 'google']);
    // Gateway allowlist.
