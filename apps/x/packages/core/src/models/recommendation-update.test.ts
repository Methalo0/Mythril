...
const mocks = vi.hoisted(() => ({
    ...
    getMythrilConfig: vi.fn(async (): Promise<unknown> => ({ modelRecommendations: {} })),
    getModelCatalog: vi.fn(async (): Promise<unknown> => ({ providers: [], defaultModel: null })),
    capture: vi.fn(),
}));

vi.mock('../config/config.js', () => ({ WorkDir: workDir }));
vi.mock('../config/mythril.js', () => ({ getMythrilConfig: mocks.getMythrilConfig }));
...
beforeEach(async () => {
    ...
    mocks.getMythrilConfig.mockImplementation(async () => ({ modelRecommendations: RECS }));
    serveConfig({ assistantModel: { provider: 'mythril', model: 'google/gemini-3.5-flash' } });
    serveCatalog('mythril', ['Auto', 'Auto-background', 'google/gemini-3.5-flash']);
});
...
describe('checkRecommendationUpdate', () => {
    it('offers the per-slot diff for the assistant\'s provider', async () => {
        ...
        expect(result.flavor).toBe('mythril');
        expect(result.providerId).toBe('mythril');
        expect(result.rows.map((r) => r.slot)).toEqual(['assistantModel', 'knowledgeGraph', 'chatTitle']);
        expect(mocks.capture).toHaveBeenCalledWith('llm_recommendation_update_shown', expect.objectContaining({ flavor: 'mythril', row_count: 3 }));
    });
    ...
    it('records a recommendation the config already matches as seen', async () => {
        ...
        expect(await checkRecommendationUpdate()).toEqual({ shouldShow: false });
        expect(Object.keys(await readState())).toEqual(['mythril']);
    });

    it('never throws: a failing config fetch is a quiet no-show', async () => {
        mocks.getMythrilConfig.mockRejectedValue(new Error('offline'));
        expect(await checkRecommendationUpdate()).toEqual({ shouldShow: false });
    });
});
...
describe('resolveRecommendationUpdate', () => {
    it('applies exactly the checked slots and records the version as seen', async () => {
        ...
        const result = await resolveRecommendationUpdate({ flavor: 'mythril', hash: shown.hash, apply: ['assistantModel', 'chatTitle'] });
        expect(result.applied).toEqual(['assistantModel', 'chatTitle']);
        expect(mocks.updateConfig).toHaveBeenCalledWith({
            assistantModel: { provider: 'mythril', model: 'Auto' },
            taskModels: { chatTitle: { provider: 'mythril', model: 'Auto-background' } },
        });
        expect(await readState()).toEqual({ mythril: shown.hash });
    ...
    it('"Not now" writes nothing but still records the version', async () => {
        ...
        const result = await resolveRecommendationUpdate({ flavor: 'mythril', hash: shown.hash, apply: [] });
    ...
    it('re-derives rows at apply time so a stale dialog cannot clobber a newer edit', async () => {
        ...
        serveConfig({ assistantModel: { provider: 'mythril', model: 'Auto' } });
        const result = await resolveRecommendationUpdate({ flavor: 'mythril', hash: shown.hash, apply: ['assistantModel', 'knowledgeGraph'] });
        expect(result.applied).toEqual(['knowledgeGraph']);
        expect(mocks.updateConfig).toHaveBeenCalledWith({
            taskModels: { knowledgeGraph: { provider: 'mythril', model: 'Auto-background' } },
        });
    });

    it('applies nothing when the recommendation version no longer matches', async () => {
        const result = await resolveRecommendationUpdate({ flavor: 'mythril', hash: 'stale', apply: ['assistantModel'] });
        expect(result.applied).toEqual([]);
        expect(mocks.updateConfig).not.toHaveBeenCalled();
    });
});
...
describe('markRecommendationSeen', () => {
    it('suppresses the prompt for the current version only', async () => {
        ...
        mocks.getMythrilConfig.mockImplementation(async () => ({
            modelRecommendations: { ...RECS, mythril: { assistantModel: 'Auto', taskModels: {} } },
        }));
