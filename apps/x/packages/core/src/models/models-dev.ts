...
// ("claude-opus-4-8"), and the mythril gateway serves OpenAI models with no
...
export function lookupReasoningFlag(
  ...
  // Unprefixed id on a gateway-ish flavor (mythril serves "gpt-5.4" bare):
...
export async function annotateReasoningFlags<T extends { id: string }>(
  ...
  return models.map((model) => {
    const reasoning = lookupReasoningFlag(index, "mythril", model.id);
