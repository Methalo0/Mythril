...
import type { ModelRecommendations } from "@x/shared/dist/mythril-account.js";
...
/**
 * The "Mythril now recommends…" prompt: when the backend's recommendation
...
async function currentRecommendations(): Promise<ModelRecommendations | undefined> {
    return (await getMythrilConfig().catch(() => null))?.modelRecommendations;
}

/**
 * Record that the user has been offered this flavor's current
 * recommendation — called at the seeding moment (provider connect / Mythril
...
async function computeUpdate(): Promise<RecommendationUpdate | null> {
    ...
    const flavor = providerId === "mythril" || providerId === "codex"
        ? providerId
