...
describe("reasoning capability index", () => {
    ...
    it("joins dotted gateway ids against dashed catalog ids", () => {
        expect(lookupReasoningFlag(index, "mythril", "anthropic/claude-opus-4.8")).toBe(true);
        expect(lookupReasoningFlag(index, "mythril", "anthropic/claude-haiku-4.5")).toBe(false);
    });

    it("matches bare unprefixed ids on gateway flavors", () => {
        expect(lookupReasoningFlag(index, "mythril", "gpt-5.4")).toBe(true);
        expect(lookupReasoningFlag(index, "mythril", "gpt-4.1")).toBe(false);
    });
    ...
    it("returns undefined for unknown models and unknown vendors", () => {
        expect(lookupReasoningFlag(index, "mythril", "mistralai/mistral-large")).toBeUndefined();
        expect(lookupReasoningFlag(index, "mythril", "some-local-model")).toBeUndefined();
    ...
    it("drops bare ids that are ambiguous across vendors", () => {
        ...
        expect(lookupReasoningFlag(clashIndex, "mythril", "shared")).toBeUndefined();
        // Vendor-qualified lookups still work.
        expect(lookupReasoningFlag(clashIndex, "mythril", "openai/shared")).toBe(true);
