...
describe('FSModelConfigRepo imageModel', () => {
    ...
    it('null clears it; the mythril provider (no providers-map entry) clears via removeProvider too', async () => {
        const repo = new FSModelConfigRepo();
        await repo.ensureConfig();
        await repo.updateConfig({ imageModel: { provider: 'mythril', model: 'google/gemini-2.5-flash-image' } });
