...
const V1_SIGNED_IN_ASSISTANT: z.infer<typeof ModelRef> = { provider: "mythril", model: "google/gemini-3.5-flash" };
 ...
 * "mythril" ref while signed out (needs auth → was skipped).
 */
function v1Override(raw: Record<string, unknown>, key: string, signedIn: boolean): Ref | undefined {
  ...
  if (ref && (ref.provider !== "mythril" || signedIn)) return ref;
  return undefined;
}
...
function v1EffectiveAssistant(raw: Record<string, unknown>, signedIn: boolean): Ref | undefined {
  ...
  if (selection && (selection.provider !== "mythril" || signedIn)) return selection;
...
export function migrateModelsConfig(rawInput: unknown, signedIn: boolean): V2 | null {
  ...
  const liveNoteEffective = v1Override(raw, "liveNoteAgentModel", signedIn)
    ?? (signedIn ? { provider: "mythril", model: V1_CURATED_LITE } : assistantModel);
  const oldTaskModels: Record<string, Ref | undefined> = {
    ...
    chatTitle: v1Override(raw, "chatTitleModel", signedIn)
      ?? (assistantModel?.provider === "mythril"
        ? { provider: "mythril", model: V1_CURATED_CHAT_TITLE }
        : assistantModel),
  };
