...
describe("Mythril gateway request attribution", () => {
    afterEach(() => {
        vi.unstubAllGlobals();
    });
    ...
    it("sends the complete turn-scoped analytics context", async () => {
        ...
        expect(Object.fromEntries(headers.entries())).toMatchObject({
            ...
            "x-mythril-use-case": "background_task_agent",
            "x-mythril-sub-use-case": "cron",
            "x-mythril-agent-name": "background-task-agent",
        });
    });
});
