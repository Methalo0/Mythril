...
import { listCloudflareImageModels, listImageModelsForProvider, listModelsForProvider } from "./models.js";
 ...
 * same way — the Mythril gateway, the ChatGPT subscription (codex), BYOK
...
export interface ModelCatalogResult {
    ...
    defaultModel: { provider: string; model: string; effort?: "low" | "medium" | "high" | "xhigh" | "ultrahigh" } | null;
}
...
interface DiscoveredProvider {
    id: string;
    flavor: string;
    /** Absent for mythril/codex — their auth lives outside models.json. */
...
/**
 * Which providers are connected right now. Mythril and ChatGPT come from
...
async function discoverProviders(): Promise<DiscoveredProvider[]> {
    ...
    // Mythril: the hosted Mythril gateway and ChatGPT-subscription providers
    // are removed — every provider here is a user-supplied BYOK key or local
    // endpoint from models.json.
...
const IMAGE_FLAVORS = new Set(["mythril", "openrouter", "google", "openai", "ollama", "openai-compatible", "nvidia", "cloudflare", "aigateway"]);
...
const IMAGE_UNFILTERABLE_FLAVORS = new Set(["ollama", "openai-compatible", "nvidia"]);
...
async function listImageModels(provider: DiscoveredProvider): Promise<string[]> {
    if (provider.id === "mythril") return await listGatewayImageModels();
    ...
    if (provider.flavor === "cloudflare") {
        // Real task metadata ("Text-to-Image") — the one provider whose image
        // list is exact instead of guessed.
        return await listCloudflareImageModels(provider.config);
    }
    if (IMAGE_UNFILTERABLE_FLAVORS.has(provider.flavor) || provider.flavor === "aigateway") {
