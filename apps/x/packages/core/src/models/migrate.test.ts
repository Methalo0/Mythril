...
describe('migrateModelsConfig', () => {
    ...
    it('signed-in with untouched bootstrap config: materializes the curated defaults', () => {
        ...
        expect(migrateModelsConfig(v1, true)).toEqual({
            ...
            taskModels: {
                ...
                liveNoteAgent: { provider: 'mythril', model: 'google/gemini-3.1-flash-lite' },
                // v1 background tasks mirrored the live-note model.
                backgroundTask: { provider: 'mythril', model: 'google/gemini-3.1-flash-lite' },
                autoPermissionDecision: { provider: 'mythril', model: 'google/gemini-3.1-flash-lite' },
    ...
    it('signed-in user with a BYOK defaultSelection: keeps it, materializes the differing task models', () => {
        ...
        expect(v2?.taskModels).toEqual({
            ...
            meetingNotes: { provider: 'mythril', model: 'google/gemini-3.5-flash' },
    ...
    it('a mythril defaultSelection is skipped while signed out (needs auth), like v1 resolution did', () => {
        const v1 = {
            provider: { flavor: 'openai', apiKey: 'sk-a' },
            model: 'gpt-5.4',
            defaultSelection: { provider: 'mythril', model: 'google/gemini-3.5-flash' },
        };
