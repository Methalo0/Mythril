...
// Mythril gateway's default on sign-in) — the same durable
...
// BYOK flavors that can build an AI SDK image model. "mythril" (the
// signed-in gateway) is the credential-less sixth path; anthropic /
// aigateway / codex have no image surface here.
type ImageFlavor = "openrouter" | "google" | "openai" | "ollama" | "openai-compatible" | "mythril" | "nvidia" | "cloudflare";
...
function makeBackend(config: z.infer<typeof LlmProvider>): ImageBackend | null {
    ...
    switch (config.flavor) {
        ...
        case "cloudflare":
            return {
                flavor: "cloudflare",
                makeImageModel: (id) => makeCloudflareImageModel(config, id),
            };
        case "nvidia":
            return {
                flavor: "nvidia",
...
const NO_IMAGE_MODEL_ERROR = "No image model configured. Pick one under model settings → Image model: choose an NVIDIA NIM, OpenRouter, Google, OpenAI, Ollama, or OpenAI-compatible provider and one of its image models.";
...
// "mythril" is the gateway (needs sign-in); any other provider id is an
...
async function resolveImageBackend(): Promise<ImageResolution> {
    ...
    if (selection.provider === "mythril") {
        if (!(await isSignedIn())) {
            return {
                ok: false,
                error: "The configured image model runs on the Mythril gateway, but you are signed out. Sign in, or pick another image model in model settings.",
            };
        }
        return {
            ok: true,
            backend: { flavor: "mythril", makeImageModel: (id) => getGatewayProvider().imageModel(id) },
...
function describeImageError(error: unknown, modelId: string, flavor: ImageFlavor): string {
    ...
    if (statusCode === 402 || message.includes("402")) {
        if (flavor === "mythril") {
            return `Your Mythril account reported a billing problem (HTTP 402) — check your plan and credits. (${message})`;
        }
    ...
    if (isModelNotFoundError(error)) {
        if (flavor === "mythril") {
            return `Image model '${modelId}' was not found on the Mythril gateway — it may not be on the gateway's image allowlist (GET /v1/llm/models?output_modalities=image lists it). Pick a listed model in model settings. (${message})`;
        }
    ...
    if (statusCode === 401 || statusCode === 403 || /unauthorized|API_KEY_INVALID|invalid.{0,10}api.?key|incorrect api key/i.test(message)) {
        if (flavor === "mythril") {
            return `The Mythril gateway rejected the request as unauthorized — your sign-in may have expired. Sign in again. (${message})`;
        }
    ...
    'generate-image': {
        ...
        execute: async (
            ...
            if (modelOverride && !MODEL_ID_SHAPE.test(modelOverride)) {
                return {
                    success: false,
                    error: `Invalid image model id '${modelOverride}'. Use the configured provider's naming — Mythril gateway / OpenRouter "vendor/model" (e.g. "google/gemini-2.5-flash-image", "x-ai/grok-imagine-image-quality"), Google "gemini-…", OpenAI "gpt-image-…", Ollama a locally pulled model name.`,
                };
            }
