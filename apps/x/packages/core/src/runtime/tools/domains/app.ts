...
import { MythrilAppManifestSchema } from "@x/shared/dist/mythril-app.js";
...
export const appNavigationTools: z.infer<typeof BuiltinToolsSchema> = {
    'app-navigation': {
        ...
        inputSchema: z.object({
            ...
            // open-app
            appId: z.string().optional().describe("App folder slug under ~/.mythril/apps (for open-app) — opens the app in the middle pane."),
            // open-view / read-view
        ...
        execute: async (input: {
            ...
            switch (input.action) {
                ...
                case 'open-app': {
                    ...
                    let appName = appId;
                    try {
                        const raw = await fs.readFile(path.join(WorkDir, 'apps', appId, 'mythril-app.json'), 'utf-8');
                ...
                case 'read-view': {
                    ...
                    try {
                        switch (view) {
                            ...
                            case 'apps': {
                                // Installed/local Mythril apps — the copilot uses this to
...
export const appDataTools: z.infer<typeof BuiltinToolsSchema> = {
    'app-read-data': {
        ...
        execute: async ({ appFolder, file }: { appFolder: string; file?: string }) => {
            try {
                ...
                try {
                    MythrilAppManifestSchema.parse(JSON.parse(await fs.readFile(path.join(dir, 'mythril-app.json'), 'utf-8')));
                } catch {
                    return { success: false, error: `No app "${appFolder}" (missing or invalid mythril-app.json).` };
                }
    ...
    'app-set-data': {
        permission: "none",
        description: "Write a Mythril App's data file — JSON its frontend reads via GET /_mythril/data/<file>. Deterministic: you supply the content, code handles the path, atomicity (temp→rename), and the app's dataContracts validation. This is how a background task refreshes an app's data — the agent RETURNS the data; never hand-write files under apps/.",
        inputSchema: z.object({
            appFolder: z.string().describe('The app folder slug under ~/.mythril/apps.'),
        ...
        execute: async ({ appFolder, file, data }: { appFolder: string; file: string; data: unknown }) => {
            try {
                ...
                const dir = path.join(WorkDir, 'apps', appFolder);
                let manifest: z.infer<typeof MythrilAppManifestSchema>;
                try {
                    manifest = MythrilAppManifestSchema.parse(JSON.parse(await fs.readFile(path.join(dir, 'mythril-app.json'), 'utf-8')));
                } catch {
                    return { success: false, error: `No app "${appFolder}" (missing or invalid mythril-app.json).` };
                }
