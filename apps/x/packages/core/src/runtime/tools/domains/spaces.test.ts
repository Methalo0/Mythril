...
import { z } from "zod";
import { mcpTools, readOnlyMcpToolNames } from "@mythril/spaces-protocol";
...
describe("parseBlobLink", () => {
    it("parses the canonical grammar with a name", () => {
        expect(
            parseBlobLink(`https://acme.mythril.space/s/${SPACE}/b/${HASH}?name=shot%20one.png`),
        ).toEqual({ address: "acme.mythril.space", spaceId: SPACE, hash: HASH, name: "shot one.png" });
    });
...
describe("resolveOrgArg", () => {
    const mythril = { id: "org-1", name: "Mythril Labs", address: "mythril.spaces.test" };
    ...
    it("defaults to the only org", async () => {
        orgsState.orgs = [mythril];
        expect(await resolveOrgArg(undefined)).toBe(mythril);
        expect(await resolveOrgArg("")).toBe(mythril);
    });

    it("requires the argument with several orgs, naming them", async () => {
        orgsState.orgs = [mythril, acme];
        await expect(resolveOrgArg(undefined)).rejects.toThrow(/"Mythril Labs", "Acme"/);
    });
    ...
    it("matches by id, name (any case), address, slug, or server name", async () => {
        ...
        expect(await resolveOrgArg("mythril labs")).toBe(mythril);
        expect(await resolveOrgArg("mythril-labs")).toBe(mythril);
        expect(await resolveOrgArg("spaces-mythril-labs")).toBe(mythril);
