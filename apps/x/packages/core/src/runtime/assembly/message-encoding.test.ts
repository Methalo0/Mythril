...
describe("middle-pane user context encoding", () => {
    ...
    it("renders an open whiteboard with the ids the whiteboard tools take, and no content", () => {
        ...
        expect(encoded).toContain(
            'Middle pane:\nState: whiteboard\nBoard: whiteboards/roadmap.excalidraw (boardId: 01BOARD) in space "Design" on org "mythril" (spaceId: 01SPACE; pass org: "mythril")',
        );
...
describe("space mentions user context encoding", () => {
    const spaceMentions = [
        { kind: "space" as const, orgId: "org-1", orgName: "mythril", spaceId: "01SPACE", name: "Design" },
        { kind: "member" as const, orgId: "org-1", orgName: "mythril", memberId: "01HARSH", displayName: "Harsh Kumar" },
    ];
    ...
    it("lists each pick with its exact id, keyed by the @ text", () => {
        ...
        expect(encoded).toContain('- @Design = space "Design" on org "mythril" (spaceId: 01SPACE)');
        expect(encoded).toContain('- @Harsh Kumar = person "Harsh Kumar" on org "mythril" (memberId: 01HARSH;');
    ...
    it("lists a board with its space, path and boardId, pointed at the whiteboard tools", () => {
        ...
        expect(encoded).toContain(
            '- @roadmap = whiteboard "roadmap" (whiteboards/roadmap.excalidraw; boardId: 01BOARD) in space "Design" on org "mythril" (spaceId: 01SPACE; whiteboard-read / whiteboard-draw with this spaceId and boardId)',
        );
    });
    ...
    it("accepts space, board and member mentions, and rejects an unknown kind", () => {
        expect(
            UserMessageContext.safeParse({
                spaceMentions: [
                    { kind: "space", orgId: "o", orgName: "mythril", spaceId: "s", name: "Design" },
                    { kind: "board", orgId: "o", orgName: "mythril", spaceId: "s", spaceName: "Design", assetId: "b", path: "whiteboards/board.excalidraw", name: "board" },
                    { kind: "member", orgId: "o", orgName: "mythril", memberId: "m", displayName: "Harsh" },
                ],
                middlePane: { kind: "whiteboard", orgId: "o", orgName: "mythril", spaceId: "s", spaceName: "Design", assetId: "b", path: "whiteboards/board.excalidraw" },
            }).success,
        ).toBe(true);
        // A board ref without its space, or without its asset id, is not addressable by the tools.
        expect(
            UserMessageContext.safeParse({
                spaceMentions: [{ kind: "board", orgId: "o", orgName: "mythril", assetId: "b", path: "whiteboards/board.excalidraw", name: "board" }],
            }).success,
        ).toBe(false);
        ...
        expect(
            UserMessageContext.safeParse({
                spaceMentions: [{ kind: "board", orgId: "o", orgName: "mythril", spaceId: "s", spaceName: "Design", path: "whiteboards/board.excalidraw", name: "board" }],
            }).success,
        ).toBe(true);
        expect(
            UserMessageContext.safeParse({
