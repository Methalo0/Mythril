...
export async function getToolPermissionMetadata(
    ...
    // Mythril: read-only operations (read/list/search) are free anywhere on
    // the drive — the assistant should answer "what's in C:\…" without a
    // permission prompt. Writes and deletes outside the workspace still ask.
    if (targets.operation === 'read' || targets.operation === 'list' || targets.operation === 'search') {
        return null;
    }
