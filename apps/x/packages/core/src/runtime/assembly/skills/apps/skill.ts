...
A *Mythril app* is a static web application the user opens inside Mythril — its
own UI on its own origin, powered by their integrations and (optionally) a
background agent. Apps live at \`~/.mythril/apps/<folder-slug>/\` and are served
...
\`\`\`
~/.mythril/apps/<folder-slug>/
├── mythril-app.json   # manifest (required)
...
## 3. Host API (same-origin, under \`/_mythril/\`)

Errors are \`{ "error": { "code", "message" } }\`. **Every non-GET request MUST
include the header \`X-Mythril-App: 1\`** — requests without it are rejected
(anti-CSRF).

**App info + theme**
\`\`\`js
const info = await (await fetch('/_mythril/app')).json();
...
\`\`\`js
// read
const data = await (await fetch('/_mythril/data/data.json')).json();
// write (atomic; contract-checked when dataContracts matches the file)
await fetch('/_mythril/data/data.json', {
  method: 'PUT',
  headers: { 'X-Mythril-App': '1', 'Content-Type': 'application/json' },
  body: JSON.stringify(payload),
});
// list
const { entries } = await (await fetch('/_mythril/data?list=.')).json();
\`\`\`

**Composio tools** (capability = the toolkit slug)
\`\`\`js
const { items } = await (await fetch('/_mythril/tools/search', {
  method: 'POST', headers: { 'X-Mythril-App': '1', 'Content-Type': 'application/json' },
  body: JSON.stringify({ toolkit: 'github', query: 'list pull requests' }),
})).json();
const result = await (await fetch('/_mythril/tools/execute', {
  method: 'POST', headers: { 'X-Mythril-App': '1', 'Content-Type': 'application/json' },
...
\`\`\`js
const r = await (await fetch('/_mythril/fetch', {
  method: 'POST', headers: { 'X-Mythril-App': '1', 'Content-Type': 'application/json' },
...
\`\`\`js
const { text } = await (await fetch('/_mythril/llm/generate', {
  method: 'POST', headers: { 'X-Mythril-App': '1', 'Content-Type': 'application/json' },
...
\`\`\`js
const { text, turnId } = await (await fetch('/_mythril/copilot/run', {
  method: 'POST', headers: { 'X-Mythril-App': '1', 'Content-Type': 'application/json' },
...
const { audioBase64, mimeType } = await (await fetch('/_mythril/voice/tts', {
  method: 'POST', headers: { 'X-Mythril-App': '1', 'Content-Type': 'application/json' },
...
const { transcript } = await (await fetch('/_mythril/voice/transcribe', {
  method: 'POST', headers: { 'X-Mythril-App': '1', 'Content-Type': 'application/json' },
...
\`\`\`js
window.addEventListener('mythril:data-change', (e) => {
  e.preventDefault();          // suppress the full reload
  refreshFromData();           // re-fetch /_mythril/data/... and re-render
});
\`\`\`
...
- Never write credentials or personal data anywhere in the app folder except
  \`data/\`.
- Never edit \`.mythril-install.json\` or \`.mythril-publish.json\`.
- Never put files under a \`/_mythril/\` path inside \`dist/\`.
...
From app code, call third-party APIs via \`/_mythril/fetch\` — never the
...
Read \`theme\` from \`/_mythril/app\` and subscribe to theme changes; style light
AND dark — never a hard-coded dark-only palette (\`prefers-color-scheme\` tracks
the OS, not Mythril):
\`\`\`js
const events = new EventSource('/_mythril/events');
