...
// when disk-loader.js/config.js load — so the MYTHRIL_WORKDIR and homedir
...
function writeSkill(folder: string, contents: string): string {
  const dir = path.join(mythrilSkillsRoot, folder);
...
describe("skills index (disk skill merge layer)", () => {
  ...
  it("refreshDiskSkills picks up skills added and removed on disk", async () => {
    ...
    fs.rmSync(path.join(mythrilSkillsRoot, "late-arrival"), { recursive: true, force: true });
