...
Load this skill when creating or modifying agents that need access to Mythril's builtin tools (shell execution, file operations, etc.).
...
The Mythril copilot has access to special builtin tools that regular agents don't typically use. These tools help the copilot assist users with file management, app workflows, and MCP integration:
...
- \`loadSkill\` - Load a Mythril skill definition into context
...
3. Running the orchestrator with \`mythrilx --agent orchestrator_name\`
