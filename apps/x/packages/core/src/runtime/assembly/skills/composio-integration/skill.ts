...
**Native connections win over Composio.** If the system prompt says a service is connected natively in Mythril (e.g. "Gmail is connected natively" or "Slack is connected natively"), do NOT use Composio for that service — follow the native routing in the system prompt instead (Gmail reading → \`app-navigation\` \`read-view\` \`view: "email"\`; Slack → the \`slack\` skill).
...
- Extract user-provided values into the correct fields (e.g., "mythrillabs/mythril" → \`owner: "mythrillabs", repo: "mythril"\`)
- For tools with empty \`properties: {}\`, pass \`arguments: {}\`
- For tools with required fields, pass all of them

### Example: GitHub Issues

User says: "Get me the open issues on mythrillabs/mythril"

1. \`composio-search-tools({ query: "list issues", toolkitSlug: "github" })\`
   → finds \`GITHUB_ISSUES_LIST_FOR_REPO\` with required: ["owner", "repo"]
2. \`composio-execute-tool({ toolSlug: "GITHUB_ISSUES_LIST_FOR_REPO", toolkitSlug: "github", arguments: { owner: "mythrillabs", repo: "mythril", state: "open", per_page: 100 } })\`
