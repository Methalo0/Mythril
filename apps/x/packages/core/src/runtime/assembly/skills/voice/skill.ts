...
Load this skill to generate spoken audio from text or transcribe audio files to text. Both tools ride the app's own voice stack — the same ElevenLabs (TTS) and Deepgram (ASR) credentials that power voice mode, or the signed-in Mythril account. **The user never needs to add API keys for these.**
...
- Mythril Apps have the same powers via their Host API (\`POST /_mythril/voice/tts\` and \`POST /_mythril/voice/transcribe\`, manifest capability \`"voice"\`) — see the \`apps\` skill when building an app that speaks or listens.
`;
