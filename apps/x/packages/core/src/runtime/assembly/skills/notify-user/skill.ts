...
### Parameters
- **\`title\`** (optional, defaults to \`"Mythril"\`) — bold headline at the top.
...
- **\`link\`** (optional) — URL to open when the user clicks the notification. Two kinds accepted:
  ...
  - **\`mythril://...\`** — opens a view inside Mythril (see deep links below)
  - If omitted, clicking the notification focuses the Mythril app.
...
Deep link into a Mythril note:
\`\`\`json
{
  "message": "Daily brief is ready",
  "link": "mythril://open?type=file&path=knowledge/Daily/2026-04-25.md"
}
\`\`\`

## Deep links: \`mythril://\`

Use these as the \`link\` parameter to land the user on a specific view in Mythril instead of an external site. URL-encode paths/names that contain spaces or special characters.

| Target | Format | Example |
|---|---|---|
| Open a file | \`mythril://open?type=file&path=<workspace-relative path>\` | \`mythril://open?type=file&path=knowledge/People/Acme.md\` |
| Open chat | \`mythril://open?type=chat\` (optional \`&runId=<id>\`) | \`mythril://open?type=chat&runId=abc123\` |
| Knowledge graph | \`mythril://open?type=graph\` | — |
| Background task view | \`mythril://open?type=task&name=<task-name>\` | \`mythril://open?type=task&name=daily-brief\` |
| Suggested topics | \`mythril://open?type=suggested-topics\` | — |
