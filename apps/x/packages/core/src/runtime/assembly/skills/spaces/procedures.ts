...
export function threadProcedure(ctx?: ThreadContext): string {
    ...
    const where = ctx
        ? `You are in a session bound to one thread in the space "${ctx.spaceName}" (spaceId ${ctx.spaceId}, thread root ${ctx.threadRootId})${ctx.org ? `, org "${ctx.org}" — pass org: "${ctx.org}" on every spaces tool call` : ""}. ` +
          "Messages here arrive because your person typed `@mythril …` in that thread; the whole room saw the ask."
        : "Your person typed `@mythril …` in a space. The message tells you the space and the thread root. The whole room saw the ask.";
    return [
        "## When invoked from a thread",
        "",
        `${where} The room reads everything you post, so your receipt is a reaction on the invoking message (its id is in the \`[@mythril …]\` header) — a reply only when the ask wants words.`,
        "",
        "- `react` 👀 before you start, and on any follow-up `@mythril` message that arrives while you work (fold those in). That is the whole \"on it\" — never post one.",
