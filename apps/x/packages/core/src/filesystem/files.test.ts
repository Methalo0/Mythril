...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mythril-files-test-"));
  workspaceDir = path.join(tmpDir, "workspace");
  process.env.MYTHRIL_WORKDIR = workspaceDir;
  vi.resetModules();
...
describe("filesystem files", () => {
  it("resolves relative paths inside MYTHRIL_WORKDIR", async () => {
  ...
  it("expands home-relative paths", async () => {
    ...
    const resolved = files.resolveFilePath("~/mythril-test.txt");

    expect(resolved.resolvedPath).toBe(path.join(os.homedir(), "mythril-test.txt"));
    expect(resolved.isInsideWorkspace).toBe(false);
  });
