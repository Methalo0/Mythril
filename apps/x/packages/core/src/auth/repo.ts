...
const ProviderConnectionSchema = z.object({
   ...
   */
  mode: z.enum(['byok', 'mythril']).optional(),
  error: z.string().nullable().optional(),
  /**
   * `mythril` only. One session, two uses (2026-09-14): the Mythril account
...
/**
 * THE reading of the `mythril` record — every "is the user signed in?"
 * answer in the app comes through these two, nowhere else (one session,
 * two uses). `mythrilSession` = a session exists at all (the identity Spaces
 * uses); `isAppSignIn` = that session is the app's, not spaces-only.
 */
export function mythrilSession(connection: ProviderConnection): { tokens: OAuthTokens; spacesOnly: boolean; error?: string } | null {
  ...
  async getClientFacingConfig(): Promise<z.infer<typeof ClientFacingConfigSchema>> {
    ...
    for (const [provider, providerConfig] of Object.entries(config.providers)) {
      clientFacingConfig[provider] = {
        // The settings/sidebar sign-in prompts key off this: for mythril it
        // is the app sign-in (a spaces-only session reads as not connected).
        connected: provider === 'mythril' ? isAppSignIn(providerConfig) : !!providerConfig.tokens,
