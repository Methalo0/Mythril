...
const MICROSOFT_CLIENT_ID =
  process.env.MYTHRIL_MICROSOFT_CLIENT_ID || 'd3f3bc8b-47a3-4674-b039-fa947053990d';
...
export async function getProviderConfig(providerName: string): Promise<ProviderConfigEntry> {
  ...
  if (providerName === 'mythril') {
    const mythrilConfig = await getMythrilConfig();
    config.discovery = {
      mode: 'issuer',
      issuer: `${mythrilConfig.supabaseUrl}/auth/v1/.well-known/oauth-authorization-server`,
    }
  }
  return config;
}
