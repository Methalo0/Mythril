...
import { IOAuthRepo, isAppSignIn, mythrilSession } from '../auth/repo.js';
...
import { applyMythrilInitialSelection, clearMythrilSelections } from '../models/mythril-selection.js';
...
/**
 * What an APP sign-in does once the Mythril session exists: seed the assistant
 ...
 */
async function afterMythrilSignIn(): Promise<string | undefined> {
  // Signing in connects the mythril provider: if no assistant model is
  ...
  await applyMythrilInitialSelection();
  captureProviderConnected('mythril');
  try {
...
/**
 * Make sure a Mythril session exists, for Spaces (one session, two uses —
 * auth/tokens.ts). A healthy session is reused as is. Otherwise the ordinary
 * `mythril` dance runs, and — since the person chose Spaces, not the app —
 ...
 * sign-in.
 */
export async function ensureMythrilSession(): Promise<void> {
  const oauthRepo = getOAuthRepo();
  const existing = mythrilSession(await oauthRepo.read('mythril'));
  ...
  const completed = new Promise<void>((resolve, reject) => {
    ...
    unsubscribe = oauthConnectBus.subscribe((event) => {
      if (event.provider !== 'mythril' || event.success) return;
      unsubscribe();
      mythrilSpacesWaiter = null;
      reject(new Error(event.error ?? 'Mythril sign-in failed'));
    });
  });
  const started = await connectProvider('mythril');
  if (!started.success) {
    mythrilSpacesWaiter = null;
    throw new Error(started.error ?? 'Mythril sign-in could not start');
  }
  await completed;
}

/**
 * Set by ensureMythrilSession for the next connectProvider('mythril') —
 ...
 */
let mythrilSpacesWaiter: (() => void) | null = null;
...
export async function connectProvider(provider: string, credentials?: { clientId: string; clientSecret: string }): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    const spacesWaiter = provider === 'mythril' ? mythrilSpacesWaiter : null;
    if (provider === 'mythril') mythrilSpacesWaiter = null;
    const forSpaces = spacesWaiter !== null;
    let mythrilSpacesOnly = false;
    if (provider === 'mythril') {
      const connection = await oauthRepo.read('mythril');
      const existing = mythrilSession(connection);
      mythrilSpacesOnly = forSpaces && !isAppSignIn(connection);
      if (!forSpaces && existing?.spacesOnly && !existing.error) {
        await oauthRepo.upsert('mythril', { spacesOnly: false });
        const userId = await afterMythrilSignIn();
    ...
    if (provider === 'google') {
      if (!credentials?.clientId || !credentials?.clientSecret) {
        // No credentials → mythril mode if the user is signed in to Mythril
        ...
        if (await isSignedIn()) {
          try {
            ...
            console.log('[OAuth] Started mythril-mode Google connect (browser opened to webapp)');
            return { success: true };
          } catch (error) {
            console.error('[OAuth] Failed to start mythril-mode Google connect:', error);
            return {
              success: false,
    ...
    const server = await openLoopback(
      ...
      async (callbackUrl) => {
        ...
        try {
          ...
          // that reaches this token exchange (mythril path returns above
          // before any local server runs); stamp mode: 'byok' so a future
          // refresh / reconnect can't get confused with a mythril entry.
          ...
          await oauthRepo.upsert(provider, {
            ...
            ...(provider === 'mythril' ? { spacesOnly: mythrilSpacesOnly } : {}),
            error: null,
          });
          ...
          // For Mythril sign-in, ensure user + Stripe customer exist before
          ...
          if (provider === 'mythril' && !mythrilSpacesOnly) {
            signedInUserId = await afterMythrilSignIn();
          }

          spacesWaiter?.();
          // Emit success event to renderer — except for a spaces-only
          // session, which is not an app sign-in (see ensureMythrilSession).
          if (!mythrilSpacesOnly) {
            emitOAuthEvent({
              provider,
              success: true,
      ...
      // can fall back since we register the actual bound port below.
      {
        ...
        validateCallback: (url) => {
          ...
          if (receivedState == null || receivedState === '') {
            return 'The sign-in response is missing its state parameter. Close this tab and retry from Mythril.';
          }
          if (receivedState !== state) {
            console.warn(`[OAuth] ${provider}: received state ${receivedState} does not match live flow state ${state || '<unset>'}`);
            return 'This sign-in attempt is no longer active. Close this tab and retry from Mythril.';
          }
          return null;
        },
...
/**
 * Complete a mythril-mode Google connect: claim the tokens parked under
 ...
 * mythril://oauth/google/done?session=<state> URL.
 */
export async function completeMythrilGoogleConnect(state: string): Promise<void> {
  try {
    console.log('[OAuth] Claiming mythril-mode Google tokens...');
    const oauthRepo = getOAuthRepo();
    ...
    console.log('[OAuth] Mythril-mode Google connect complete');
  } catch (error) {
    console.error('[OAuth] Failed to complete mythril-mode Google connect:', error);
    emitOAuthEvent({
      provider: 'google',
      success: false,
...
export async function disconnectProvider(provider: string): Promise<{ success: boolean }> {
  try {
    ...
    // For mythril-mode Google, best-effort revoke at Google before clearing
    ...
    if (provider === 'google') {
      const connection = await oauthRepo.read(provider);
      if (connection.mode === 'mythril' && connection.tokens?.access_token) {
        try {
    ...
    if (provider === 'mythril') {
      ...
      analyticsReset();
      // Signing out disconnects the mythril provider: drop the model
      // selections that reference it (same dangling-ref cleanup as removing
      // any provider). The composer prompts for a new pick.
      await clearMythrilSelections();
      captureProviderDisconnected('mythril');
    }
...
export async function disconnectGoogleIfScopesStale(): Promise<void> {
  try {
    ...
    // Best-effort revoke at Google for mythril-mode grants (mirrors disconnectProvider).
    if (connection.mode === 'mythril' && connection.tokens.access_token) {
      try {
        const revokeUrl = `https://oauth2.googleapis.com/revoke?token=${encodeURIComponent(connection.tokens.access_token)}`;
        const res = await fetch(revokeUrl, { method: 'POST', signal: AbortSignal.timeout(5000) });
