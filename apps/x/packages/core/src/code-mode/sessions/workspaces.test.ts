...
const initial: CodeSession = {
    ...
    worktree: { path: '/tmp/wt', branch: 'mythril/s1', baseBranch: 'main', baseCommit: 'original' }, createdAt: '2026-09-01T00:00:00.000Z',
};
...
describe('shared worktree sessions', () => {
    ...
    it('removes the worktree only with its final session', async () => {
        const { service } = setup();
        await service.delete('s1', { removeWorktree: true, deleteBranch: true });
        expect(mocks.worktreeRemove).toHaveBeenCalledWith('/tmp/project', '/tmp/wt', { force: true, deleteBranch: 'mythril/s1' });
    });
    ...
    it('changes the base across empty sibling sessions and preserves conversations', async () => {
        ...
        expect(mocks.changeWorktreeBase).toHaveBeenCalledWith('/tmp/wt', 'mythril/s1', 'original', 'release');
