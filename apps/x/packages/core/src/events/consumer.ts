...
export interface EventConsumer {
     ...
     */
    findCandidates(event: MythrilEvent, targets: EventConsumerTarget[]): Promise<string[]>;

    /** Fire the consumer's agent on a single candidate id. */
    fireCandidate(event: MythrilEvent, id: string): Promise<EventConsumerFireResult>;
}
