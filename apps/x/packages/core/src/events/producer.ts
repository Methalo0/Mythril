...
import type { MythrilEvent } from '@x/shared/dist/events.js';
...
/**
 * Write a MythrilEvent to the events/pending/ directory. The filename is the
 ...
 */
export async function createEvent(event: Omit<MythrilEvent, 'id'>): Promise<void> {
    fs.mkdirSync(PENDING_DIR, { recursive: true });

    const idGen = container.resolve<IMonotonicallyIncreasingIdGenerator>('idGenerator');
    const id = await idGen.next();

    const fullEvent: MythrilEvent = { id, ...event };
