...
// The headless-safe half of Mythril's boot: everything that runs schedulers,
...
// the standalone mythril-server boot the SAME service set in the SAME order —
...
export async function initCoreServices(): Promise<void> {
  ...
  // Mythril Apps server (per-app origins on 127.0.0.1:3210).
  registerAppsHostApi();
  ...
  // Mythril: Home/Todo surface removed — skip its session repair and the
  // morning-planner background task.
  ...
  // Mythril: Email and Meetings surfaces were removed, so their sync
  // pipelines (Gmail/Outlook/Calendar/Fireflies/Granola), meeting prep, and
  // calendar notifications are dead weight — each was a polling loop holding
  // data in memory. Disabled to cut RAM and idle CPU.
  void initGmailSync; void initOutlookSync; void initCalendarSync;
  void initOutlookCalendarSync; void initFirefliesSync; void initGranolaSync;
  void initCalendarNotifications; void initMeetingPrep; void initChromeSync;
  initGraphBuilder();
  initNoteTagging();
  initInlineTasks();
  initAgentRunner();
