...
describe('todo fileops parse/serialize', () => {
    ...
    it('normalizes keys by whitespace and case', () => {
        expect(normalizeKey('  @Mythril   Research pricing MODELS ')).toEqual('@mythril research pricing models');
    });
...
`;
        ...
        expect(deck.children.map(c => [c.text, c.checked, c.delegated])).toEqual([
            ['@mythril research TAM', true, true],
            ['research competitors', false, false],
        ]);
        expect(deck.children[0].key).toEqual(subKey('create pitch deck', '@mythril research TAM'));
...
`;
        ...
        expect(entries.map(e => [e.item.text, e.date, e.item.checked])).toEqual([
            ['@mythril research pricing', '2026-07-27', true],
    ...
    it('detects @mythril mentions as delegation', () => {
        expect(isDelegated('@mythril do the thing')).toBe(true);
        expect(isDelegated('email arjun@mythrillabs.com')).toBe(false);
