...
// ---------------------------------------------------------------------------
// One rolling list at ~/.mythril/todo.md — the file is the whole truth.
...
const PROPOSED_RE = /\s*\(via mythril\)\s*$/i;
...
function serializeItem(item: TodoItem): string[] {
    const mark = (i: TodoItem) => (i.proposed ? ' (via mythril)' : '');
    const out = [
...
- [ ] @mythril introduce yourself — what can you do here?
