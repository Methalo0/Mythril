...
async function driveTurn(
    ...
    message: string,
    subUseCase: string,
    modelOverride?: { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'ultrahigh' },
    autoPermission = true,
): Promise<TodoRunResult> {
    ...
    try {
        ...
        try {
            ...
            const selection: { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'ultrahigh' } =
        ...
        for (;;) {
            ...
            if (!surfacedSuspension) {
                ...
                void notifyIfEnabled('agent_permission', {
                    title: 'Mythril needs an approval',
                    message: itemText,
                    link: 'mythril://open?type=home',
                });
            }
 ...
 * (typing @mythril, the run chip, retry). Fire-and-forget from IPC:
...
export async function replyHomeChat(
    sessionId: string,
    message: string,
    opts?: { model?: { provider: string; model: string; effort?: 'low' | 'medium' | 'high' | 'xhigh' | 'ultrahigh' }; autoPermission?: boolean },
): Promise<HomeChatResult> {
