...
// Auth precedence mirrors the renderer's voice mode: signed-in Mythril
...
export async function transcribeAudio(audio: Buffer, opts?: { mimeType?: string }): Promise<{ transcript: string }> {
    ...
    const config = await getVoiceConfig();
    if (!config.deepgram) {
        throw new Error(`Deepgram not configured. Sign in to Mythril, or create ${path.join(WorkDir, 'config', 'deepgram.json')} with { "apiKey": "<your-key>" }`);
    }
...
async function transcribeViaProxy(audio: Buffer): Promise<{ transcript: string }> {
    ...
    if (!websocketApiUrl) throw new Error('Mythril websocket API URL is not configured');
    const accessToken = await getAccessToken();
