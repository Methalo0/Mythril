...
function mockConfig(creditActivations: unknown[] | null = ACTIVATIONS) {
  vi.doMock("../config/mythril.js", () => ({
    getMythrilConfig: vi.fn(async () => ({
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mythril-credits-test-"));
  process.env.MYTHRIL_WORKDIR = tmpDir;
  // enable the feature flag via the dev override (no PostHog in tests)
  process.env.MYTHRIL_CREDITS = "1";
  vi.resetModules();
