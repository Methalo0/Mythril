 ...
 * for non-matching users. Evaluated against the identified mythril user and
 * cached briefly so reward triggers don't add a network hop. MYTHRIL_CREDITS
 * env var (1/0) overrides everything for development.
 */
export async function isCreditsEnabled(): Promise<boolean> {
  const override = process.env.MYTHRIL_CREDITS;
...
// Claimed-state cache, keyed by Mythril user id so switching accounts on the
...
/**
 * Extract the Supabase user id (`sub` claim) from the Mythril access token
...
export async function claimReferralCode(rawCode: string): Promise<ReferralClaimResult> {
  ...
  try {
    ...
    if (!(await isSignedIn())) return { ok: false, message: 'Sign in to Mythril to use an invite code.' };
    const accessToken = await getAccessToken();
    const userId = userIdFromToken(accessToken);
    if (!userId) return { ok: false, message: 'Sign in to Mythril to use an invite code.' };
