...
// Mythril Apps schemas (spec §4.2, §5.1, §9.1, §11.4, §12.2).

export const PACKAGE_NAME_RE = /^[a-z0-9]+(-[a-z0-9]+)*$/;
export const SEMVER_RE = /^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)$/;

// ---------------------------------------------------------------------------
// Manifest — mythril-app.json (§4.2)
// ---------------------------------------------------------------------------

export const MythrilAppManifestSchema = z.object({
    ...
    capabilities: z.array(z.string()).default([])
        .describe('Capability identifiers this app may use (D7): Composio toolkit slugs for /_mythril/tools/*, plus the reserved identifiers "llm" (§7.6), "copilot" (§7.7), and "voice" (/_mythril/voice/* TTS + transcription). Empty = none.'),
    ...
    build: z.object({ command: z.string() }).optional()
        .describe('RESERVED. Mythril MUST NOT execute this in V1.'),
    minMythrilVersion: z.string().regex(SEMVER_RE).optional()
        .describe('RESERVED. Minimum compatible Mythril version; not enforced in V1.'),
}).passthrough(); // unknown fields survive round-trips (forward compatibility)

export type MythrilAppManifest = z.infer<typeof MythrilAppManifestSchema>;

// ---------------------------------------------------------------------------
// Install record — .mythril-install.json (§12.2)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Publish record — .mythril-publish.json (§11.4)
// ---------------------------------------------------------------------------
...
export const AppSummarySchema = z.object({
    ...
    manifest: MythrilAppManifestSchema.optional(), // present when ok
