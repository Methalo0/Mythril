...
export const ReasoningEffort = z.enum(["low", "medium", "high", "xhigh", "ultrahigh"]);
...
export const LlmProvider = z.object({
  // "mythril" (signed-in gateway) and "codex" (ChatGPT subscription via
  ...
  flavor: z.enum(["openai", "anthropic", "google", "openrouter", "aigateway", "ollama", "openai-compatible", "nvidia", "cloudflare", "mythril", "codex"]),
  apiKey: z.string().optional(),
  // Optional pool of extra keys for the same provider. When present,
  // requests rotate across [apiKey, ...apiKeys] with a per-key sliding
  // window cap (see core/models/key-rotation.ts); on 429/401 the request
  // retries with the next key. NVIDIA NIM keys are capped at 35 req/min
  // (their hard limit is 40).
  apiKeys: z.array(z.string()).optional(),
  // Cloudflare Workers AI: the account the API token belongs to. Builds the
  // OpenAI-compatible base URL .../accounts/{accountId}/ai/v1.
  accountId: z.string().optional(),
  // Cloudflare: also list models flagged require_workers_paid (off by default).
  includePaid: z.boolean().optional(),
  baseURL: z.string().optional(),
  headers: z.record(z.string(), z.string()).optional(),
  // Context window (in tokens) to request from local runtimes. Ollama defaults
  // to a ~4k window that silently truncates Mythril's prompts; when unset,
...
// "mythril" / "codex" for the credential-less providers. Today one instance
...
export const LlmModelConfig = z.object({
  ...
  // with the gateway's image model on Mythril sign-in; while absent the
