...
import { AppSummarySchema, RegistryRecordSchema, MythrilAppManifestSchema } from './mythril-app.js';
...
export const ipcSchemas = {
  ...
  // provider — Mythril gateway, ChatGPT subscription (codex), BYOK keys,
  ...
  'models:list': {
    ...
    res: z.object({
      providers: z.array(z.object({
        ...
        id: z.string(),
        // Provider TYPE ("openai", "ollama", "mythril", "codex", …) —
  ...
  // The "Mythril now recommends…" prompt (core/models/recommendation-update):
  ...
  // selection (the Mythril sign-in path does the same main-side).
  ...
  // consumers go through the useMythrilConfig() hook.
  'mythril:getConfig': {
    req: z.null(),
    res: MythrilApiConfig.nullable(),
  },
  'oauth:didConnect': {
    req: z.object({
      provider: z.string(),
  ...
  // Mythril Apps (spec §13) — M1 local channels.
  'apps:serverStatus': {
    req: z.object({}),
    res: z.object({
      running: z.boolean(),
  ...
  'apps:catalogDetail': {
    ...
    res: z.object({
      ...
      manifest: MythrilAppManifestSchema.optional(),
      readme: z.string().optional(),
  ...
  // Managed OAuth-redirect Picker: the Mythril backend runs the pick with the
  ...
  'todo:addItem': {
    req: z.object({
      text: z.string(),
      // Fire the item's run immediately (composer delegate / @mythril typed).
      run: z.boolean(),
      ...
      model: z.object({
        provider: z.string(),
        model: z.string(),
        effort: z.enum(['low', 'medium', 'high', 'xhigh', 'ultrahigh']).optional(),
      }).optional(),
  ...
  // (with its via-mythril badge); recorded as a positive 'kept' signal.
  ...
  // Mythril server (phone pairing) channels — client-local: answered by main,
  ...
  // mythril-server saved from Settings. Env vars override and lock the UI.
  ...
  // Wire contract: @mythril/spaces-protocol (apps/harbor/CONTRACT.md).
  ...
  // One session, two uses (2026-09-14): the Mythril account IS the identity
  ...
  // out of the app leaves a spaces-only session). signInMythril is the Spaces
  ...
  // @mythril in a thread (spec §8): the renderer detected an addressed message
  ...
  'spaces:invokeMythril': {
    req: z.object({
      ...
      // when the draft addresses @mythril). Absent = the assistant's defaults.
      options: z
        .object({
          model: z.object({ provider: z.string(), model: z.string(), effort: z.enum(['low', 'medium', 'high', 'xhigh', 'ultrahigh']).optional() }).optional(),
          permissionMode: z.enum(['auto', 'manual']).optional(),
          searchEnabled: z.boolean().optional(),
          codeMode: z.enum(['claude', 'codex']).optional(),
        })
  ...
  // (pre-index post, or not this member's Mythril) — the caller may fall
  ...
  // topic→session registry is local, so only the member whose Mythril runs
