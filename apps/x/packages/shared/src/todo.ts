...
// ---------------------------------------------------------------------------
//
// One rolling markdown file at `~/.mythril/todo.md` shared between the user
// and @mythril. Everything durable lives in the file — there is no sidecar
// state, no hidden anchors. GFM task lines are items; typing `@mythril` in a
...
export type TodoItem = {
    ...
    key: string;
    /** Raw line text after the checkbox (includes any @mythril mention). */
    text: string;
    checked: boolean;
    /** True when the text mentions @mythril. */
    delegated: boolean;
    /** True when the planner suggested this item ("(via mythril)" marker on
