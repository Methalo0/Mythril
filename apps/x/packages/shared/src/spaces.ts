import { z } from 'zod';
import { addressesMythril, mapMentionTokens, mentionsAsText } from '@mythril/spaces-protocol';
import type {
  AcceptInviteResult,
  Asset,
  BlobInfo,
...
// truth is @mythril/spaces-protocol (see apps/harbor/CONTRACT.md) — this file
...
export const SpacesOrgSummary = z.object({
  ...
  memberId: z.string(),
  /** `session` = a managed org riding the Mythril account session (one session, two uses — 2026-09-14). */
  authKind: z.enum(['dev', 'oauth', 'session']),
  /** Present = the org needs a re-login (refresh dead, or no Mythril session). Visible and gentle, never silent. */
...
/**
 * One thread where this member's Mythril is working (or waiting to) on a
...
export type { MentionRef, MentionStamps, OrgLink } from '@mythril/spaces-protocol';

/** Does the body deliberately address @mythril — a token, never the bare word (spec §8)? */
export function containsMythrilAddress(body: string): boolean {
  return addressesMythril(body);
}
