...
export type LiveNote = {
    ...
    model?: string;
    provider?: string;
    effort?: "low" | "medium" | "high" | "xhigh" | "ultrahigh";
...
export const LiveNoteSchema = z.object({
    ...
    effort: z.enum(['low', 'medium', 'high', 'xhigh', 'ultrahigh']).optional().describe('ADVANCED — leave unset. Reasoning effort paired with the per-note model override; unset means Auto (provider default). Only meaningful alongside `model`.'),
...
// `./events.ts` as `MythrilEventSchema` / `Pass1OutputSchema`. These re-exports
// keep older import paths working for one release; remove after nothing imports
// them from here.
export { MythrilEventSchema as KnowledgeEventSchema, Pass1OutputSchema } from './events.js';
export type { MythrilEvent as KnowledgeEvent, Pass1Output } from './events.js';
